...
1. **Commit after each unit of work** - Don't let multiple logical changes accumulate
1. If working on a feature, ensure you're on a feature branch, not `main` or `dev`
1. See [CONTRIBUTORS.md](CONTRIBUTORS.md) and [SETUP_GUIDE.md](SETUP_GUIDE.md) for full workflow
...
All code changes must ALWAYS follow TDD (no exceptions unless the user explicitly waives it):

1. **RED**: Write a failing test first (use `@pytest.mark.red_phase` if needed).
1. **GREEN**: Implement the minimal code to pass.
...
1. Update test counts when they significantly change
1. Add new issues/TODOs discovered during work
1. Break large changes into discrete, trackable items
1. Reference file paths for implemented features
...
- **TDD (ALWAYS)**: Red-Green-Refactor cycle mandatory for every code change unless explicitly waived
...
1. **Function docstring**: One-line summary + Args + Returns + minimal Example
1. **RST file** (`docs/reference/*.rst`): Full LaTeX formulas, historical context, code examples
