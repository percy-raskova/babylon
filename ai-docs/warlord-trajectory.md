...
The bourgeoisie wealth collapsed to near-zero while Labor Aristocracy accumulated massively. Initially flagged as **ADR037: Critical Bug** - this contradicts Marxist political economy where capital concentrates upward during crisis.

## The Key Insight

**The dynamics model a historically valid trajectory: the Warlord Coup.**
...
Common pattern: Military/police forces realize they have the actual power and seize it when civilian leadership can no longer maintain control.

### The Private Prison Industry (American Context)
...
The enforcers don't need a coup - they become the new bourgeoisie through the carceral apparatus itself.
...
```
The bourgeoisie fix payment (through extreme extraction) and maintain control. This is the "pure" necropolis ruled by capitalists.
...
```
The enforcers establish a military junta that rules the necropolis directly. This is the "warlord era."
...
IF can_pay:
    → TRAJECTORY_A: Classical Concentration
ELSE IF enforcer_consciousness > COUP_THRESHOLD:
    → TRAJECTORY_B: Warlord Coup
ELSE:
    → FAILED_STATE: System collapses into chaos
```

## Player Agency Implications (Epoch 2)
...
The Warsaw Ghetto Dynamic still applies: when P(S|A) → 0, revolution becomes the only rational choice.
...
- **ADR037** in `ai-docs/decisions.yaml`: Original bug report, now deferred to Epoch 2
- `ai-docs/carceral-equilibrium.md`: The 70-year trajectory theory
- `ai-docs/terminal-crisis-dynamics.md`: Endgame mechanics
- `brainstorm/warlord-trajectory.md`: Full Epoch 2 expansion proposal with implementation details

## Status

**Discovered**: January 1, 2026 (Session #139)
