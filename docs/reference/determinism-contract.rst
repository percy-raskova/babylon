...
.. list-table::
   ...
   * - Full-trace goldens (``tests/baselines/`` dense per-tick traces,
       Program 13 item 2)
     - Tolerance-bounded, per Constitution III.12 corollary (c)
     - ``[PENDING CODE]`` at time of writing — not yet authored; will extend
       regime 1 to every tick, not just checkpoint ticks.
