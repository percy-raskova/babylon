...
- `PracticeAuthorityKindV1` is exactly `PLAYER_SEAT = 1` and `DETERMINISTIC_POLICY = 2`. `PracticeTargetDomainV1` is the stable wire code table naming closed `NodeType` semantics; its only grounded V1 member is `SOCIAL_CLASS = 1`. This groundwork serializes that contract-local code but does not resolve a graph target. Gate 5/PER-27 owns the admission adapter that must resolve the textual `NodeType/SOCIAL_CLASS` member through `LoadedScenario.vocabulary`, require `GraphSubstrate::node_type_of(target) == "SOCIAL_CLASS"`, and refuse a missing or differently typed target. No layer serializes a scenario-local BSL enum ordinal.
...
- [ ] **Step 1: Write budget/topology/defines red tests.** Cover `0`, `-0.0`,
  ...
  not accept a
  `PRESENCE`/`MEMBERSHIP`/`SOLIDARITY` topology as execution authorization.
