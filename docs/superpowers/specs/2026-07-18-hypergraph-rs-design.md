...
| D12 | `freeze()` monkey-patches a fixed method list that omits `clear_edges` — a FROZEN network can still have every edge deleted | The `frozen` flag guards ALL structural mutators uniformly, `clear_edges` included — a freeze that permits wholesale edge deletion is not a freeze | pass-through (XGI's omission is a swizzling artifact, not semantics) |
