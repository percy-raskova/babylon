...
```mermaid
flowchart LR
    T0["T0: Theory refoundation"] --> T1["T1: Relational territory dossier"]
    T1 --> T2["T2: Situated practice circuit"]
    G3["G3: committed Archive and fog"] --> T2
    G5["G5: next-week player intent"] --> T2
    G6["G6: inventory, freight, and class flow"] --> MA["Material Mutual Aid"]
    G7["G7: exact money and escrow, when used"] -.-> MA
    T2 --> T3["T3: Emergent-shape evidence"]
    MA --> T3
    AM["PER-44 membership contract + encounter producer"] --> T3
    T3 --> A["Archive evaluation artifact"]
```
