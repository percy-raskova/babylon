...
```mermaid
flowchart LR
  subgraph bloc["Ruling bloc / sovereign"]
    S[sovereign]
  end
  subgraph app["institution = apparatus (hyperedge)"]
    A1[["religious apparatus"]]
    A2[["educational apparatus"]]
    A3[["communications apparatus"]]
  end
  O1[organization: church]
  O2[organization: university]
  O3[organization: newspaper]
  O4[organization: union]

  O1 -- "membership: integration degree" --> A1
  O2 -- "membership: integration degree" --> A2
  O2 -. "personnel-selection channel" .-> S
  O3 -- "membership" --> A3
  O4 -- "membership (low / contested)" --> A2
  A1 -- "consent produced for" --> S
  S -- "protection + subsidy" --> O1
```
