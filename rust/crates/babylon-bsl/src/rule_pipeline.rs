...
// Task 2 (issue #652 §2.1d) RED probe. Temporary home — moves into
// `error_identity.rs`'s own `#[cfg(test)] mod tests` once that module
// exists (GREEN); kept here for now only because `identity_of` doesn't
// exist anywhere yet to host it against.
#[cfg(test)]
mod identity_of_red_probe {
    use super::LoadError;
    use crate::declarations::DeclError;
    use crate::error_identity::{identity_of, ErrorIdentity};
    use crate::grammar::GrammarError;
