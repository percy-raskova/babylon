...
(rule event/wealth-aggregates
  ...
  (when (and (>= (fold sum (nodes NodeType/SOCIAL_CLASS) (field-of it social-class/wealth)) 550$)
             (>= (fold max (nodes NodeType/SOCIAL_CLASS) (field-of it social-class/wealth)) 500$)
             (<= (fold min (nodes NodeType/SOCIAL_CLASS) (field-of it social-class/wealth)) 50$)
             (>= (fold mean (nodes NodeType/SOCIAL_CLASS)
                   (field-of it social-class/agitation)
                   :weight (field-of it social-class/population)) 0.3i)))
  (effects
