...
fn exact_reader_footprint(held: &[String], markers: (bool, bool, bool, bool)) -> bool {
    let (atoms, verification, economy, material) = markers;
    let mut expected = if atoms {
...
fn reader_schema_markers(
    client: &mut postgres::Client,
) -> Result<(bool, bool, bool, bool), SemanticArchiveReaderErrorV1> {
    let row = client
