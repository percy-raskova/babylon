...
use crate::committed_tick_envelope::{
    CommittedTickEnvelopeV1, CommittedTickRowFamiliesV1, CommittedTickRowV1,
};
...
pub(crate) struct StoredTypedTickV1 {
...
impl StoredTypedTickV1 {
    pub(crate) const fn envelope(&self) -> &CommittedTickEnvelopeV1 {
        &self.envelope
    }
...
pub(crate) fn read_stored_typed_tick_v1(
    client: &mut impl GenericClient,
    campaign_id: CampaignId,
    resolve_tick: u64,
    scenario_scope: &str,
) -> Result<Option<StoredTypedTickV1>, RustPersistenceRuntimeErrorV1> {
    let resolve_tick_sql =
        i64::try_from(resolve_tick).map_err(|_| RustPersistenceRuntimeErrorV1::CampaignConflict)?;
...
fn read_graph_nodes(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<GraphNodeRowsV1, RustPersistenceRuntimeErrorV1> {
    client
        .query(
...
fn read_graph_node_f64(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<GraphNodeF64RowsV1, RustPersistenceRuntimeErrorV1> {
    client
        .query(
...
fn read_graph_edges(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<GraphEdgeRowsV1, RustPersistenceRuntimeErrorV1> {
    client
        .query(
...
fn read_graph_hyperedges(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<GraphHyperedgeRowsV1, RustPersistenceRuntimeErrorV1> {
    client
        .query(
...
fn read_graph_edge_f64(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<GraphEdgeF64RowsV1, RustPersistenceRuntimeErrorV1> {
    client
        .query(
...
fn read_graph_node_currency(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<GraphNodeCurrencyRowsV1, RustPersistenceRuntimeErrorV1> {
    client
        .query(
...
fn read_graph_hyperedge_f64(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<GraphHyperedgeF64RowsV1, RustPersistenceRuntimeErrorV1> {
    client
        .query(
...
fn read_material_rows(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<MaterialStateRowsV1, RustPersistenceRuntimeErrorV1> {
    let world_registers = client
        ...
        .collect::<Result<Vec<_>, RustPersistenceRuntimeErrorV1>>()?;
...
fn read_territories(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<Vec<TerritoryStateRowV1>, RustPersistenceRuntimeErrorV1> {
...
fn read_organizations(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<Vec<OrganizationStateRowV1>, RustPersistenceRuntimeErrorV1> {
...
fn read_event_rows(
    ...
    for row in client
        .query(
            "SELECT ordinal, position, field_name, value_tag, int_value, currency_value::text, \
                    real_bits, ratio_bits, ratio_min_bits, ratio_max_bits, bool_value, enum_type, \
                    enum_member, stable_key \
             FROM babylon_state.tick_event_field_v1 \
    ...
    for row in client
        .query(
            "SELECT ordinal, event_type FROM babylon_state.tick_event_v1 \
        ...
        .map_err(|_| database("read stored events"))?
    {
        let ordinal: i64 = decode_column(&row, 0)?;
        let ordinal_u32 =
            u32::try_from(ordinal).map_err(|_| RustPersistenceRuntimeErrorV1::CampaignConflict)?;
...
fn read_checkpoint_rows(
...
) -> Result<(Vec<CommittedTickRowV1>, Vec<Vec<u8>>), RustPersistenceRuntimeErrorV1> {
    let manifest = client
        .query_opt(
    ...
    for (index, row) in stored.iter().enumerate() {
        ...
        let tag_u8 =
            u8::try_from(tag).map_err(|_| RustPersistenceRuntimeErrorV1::CampaignConflict)?;
...
fn read_archive_receipt(
    ...
    campaign_id: CampaignId,
    resolve_tick: i64,
) -> Result<CommittedTickRowV1, RustPersistenceRuntimeErrorV1> {
    let row = client
        .query_opt(
...
fn decode_bsl_value(
    row: &Row,
    start: usize,
) -> Result<StableBslValueV1, RustPersistenceRuntimeErrorV1> {
    ...
    match tag {
        ...
        6 => Ok(StableBslValueV1::Enum {
            enum_type: enum_type.ok_or(RustPersistenceRuntimeErrorV1::CampaignConflict)?,
            member: enum_member.ok_or(RustPersistenceRuntimeErrorV1::CampaignConflict)?,
        }),
        7 => Ok(StableBslValueV1::Node(decode_stable_key(
            &stable_key.ok_or(RustPersistenceRuntimeErrorV1::CampaignConflict)?,
        )?)),
        ...
        _ => Err(RustPersistenceRuntimeErrorV1::CampaignConflict),
    }
}

fn decode_stable_key(bytes: &[u8]) -> Result<StableElementKeyV1, RustPersistenceRuntimeErrorV1> {
    StableElementKeyV1::from_canonical_bytes(bytes)
        .map_err(|_| RustPersistenceRuntimeErrorV1::CampaignConflict)
}

fn graph_row_count(graph: &StableGraphStateV1) -> Result<u32, RustPersistenceRuntimeErrorV1> {
    let rows = graph.rows();
    let count = [
        rows.nodes().len(),
        rows.node_f64().len(),
    ...
    .ok_or(RustPersistenceRuntimeErrorV1::CampaignConflict)?;
    u32::try_from(count).map_err(|_| RustPersistenceRuntimeErrorV1::CampaignConflict)
}

fn require_position(actual: i32, expected: usize) -> Result<(), RustPersistenceRuntimeErrorV1> {
...
fn decode_digest(row: &Row, index: usize) -> Result<[u8; 32], RustPersistenceRuntimeErrorV1> {
