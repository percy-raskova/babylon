...
fn identity(local_id: &str, domain: &str, authority: &str) -> TypedIdentity {
    TypedIdentity {
...
fn test_identity(local_id: &str) -> TypedIdentity {
    identity(local_id, "entity", "test")
}

fn registry_identity(domain: &str, local_id: &str) -> TypedIdentity {
    identity(local_id, domain, "babylon.rtd.v1")
}

fn surface() -> DecisionSurface {
    DecisionSurface {
...
fn base_draft() -> RtdDossierDraft {
    RtdDossierDraft {
        schema: RTD_SCHEMA_ID.to_owned(),
        schema_version: 1,
        projection_version: 1,
        audience: Audience::AdminMaterial,
...
fn reference(local_id: &str, digest: &str) -> ReferenceDigest {
    ReferenceDigest {
...
fn county_facet() -> Facet {
    Facet {
        facet_id: identity("facet-1", "facet", "test"),
        family: FacetFamily::ProductionCirculation,
        ...
        value_kind: ValueKind::Uint64Bits,
...
fn with_county_facet(facet: Facet) -> RtdDossierDraft {
...
fn lodes_facet(flow_id: TypedIdentity) -> Facet {
    Facet {
...
fn commuter_flow(flow_id: TypedIdentity, payload: Vec<TypedIdentity>) -> ReferenceFlow {
    ReferenceFlow {
        flow_id,
        flow_kind: FlowKind::CommuterJobs,
...
fn assert_error(expected: RtdError, draft: &RtdDossierDraft) {
...
fn untrusted_json_boundary_classifies_shape_and_normalizes_float_negative_zero() {
    ...
    facet.value_kind = ValueKind::Float64Bits;
...
fn metric_registry_and_required_digest_semantics_refuse_mutations() {
    ...
    assert_error(RtdError::NativeGrain, &draft);
    let mut draft = with_county_facet(county_facet());
    draft.facets[0].coordinates.push(DimensionCoordinate {
    ...
    assert_error(RtdError::NativeGrain, &draft);
    let mut draft = with_county_facet(county_facet());
    draft.facets[0].evidence_class = EvidenceClass::Designed;
    assert_error(RtdError::NativeGrain, &draft);
    let mut draft = with_county_facet(county_facet());
...
fn every_relation_binding_and_payload_mutation_is_checked() {
    ...
    command_draft.dyads.push(dyad(DyadKind::Command));
    let mut border = commuter_flow(identity("border", "flow", "test"), Vec::new());
    border.flow_kind = FlowKind::BorderSynthesis;
    ...
    let mut wrong = dyad(DyadKind::Presence);
    ...
    let mut command = dyad(DyadKind::Command);
    ...
    let mut wrong_evidence = dyad(DyadKind::Solidarity);
    wrong_evidence.evidence_class = EvidenceClass::Observed;
...
fn geography_and_administration_refusals_are_exact() {
    ...
    let mut draft = base_draft();
    draft.audience = Audience::PlayerKnowledge;
    assert_error(RtdError::ForbiddenReduction, &draft);
    let mut draft = base_draft();
    draft.durability = Durability::Committed;
    assert_error(RtdError::ForbiddenReduction, &draft);
    let mut draft = base_draft();
    ...
    assert_error(RtdError::ForbiddenReduction, &draft);
    let mut flattened = dyad(DyadKind::Command);
    ...
    let edge = Hyperedge {
        ...
        hyperedge_kind: HyperedgeKind::PublicRelation,
...
fn membership_and_h3_gap_refusals_are_exact() {
    let membership = ScaleMembership {
        membership_id: identity("msa", "membership", "test"),
        member_ref: identity("26163", "county", "census"),
        scale_ref: identity("region", "scale", "test"),
        membership_kind: MembershipKind::WeightedOverlap,
        status: Status::Present,
        weight_status: Status::Absent,
    ...
    draft.scale_memberships[0].membership_kind = MembershipKind::Metropolitan;
    assert_error(RtdError::MsaEvidence, &draft);
    let gap = Gap {
        ...
        reason_code: GapReason::IdentityContractPending,
...
fn append_uses_every_closed_collection_limit_without_mutating_failure_input() {
    let kinds = [
        (RtdCollectionKind::Focus, 64),
        (RtdCollectionKind::ReferenceDigests, 4_096),
        (RtdCollectionKind::ScaleMemberships, 65_535),
        (RtdCollectionKind::Facets, 65_535),
        (RtdCollectionKind::Dyads, 65_535),
        (RtdCollectionKind::Hyperedges, 65_535),
        (RtdCollectionKind::Flows, 65_535),
        (RtdCollectionKind::Gaps, 65_535),
        (RtdCollectionKind::Provenance, 65_535),
        (RtdCollectionKind::Coordinates, 32),
        (RtdCollectionKind::MemberRefs, 1_024),
        (RtdCollectionKind::PayloadFacets, 256),
        (RtdCollectionKind::DisplayRefs, 256),
        (RtdCollectionKind::ProvenanceRefs, 8_192),
    ];
    for index in 0..14 {
...
fn valid_json() -> String {
    format!(
        ...
        schema = RTD_SCHEMA_ID,
        zero = ZERO_DIGEST,
    )
}

fn facet_json(facet: &Facet) -> String {
