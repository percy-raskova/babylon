...
- [x] CHK006 - Is the consecutive-period counter reset behavior explicitly specified when a county exits crisis (enters recovery) and later re-enters crisis? **RESOLVED**: data-model.md CrisisState invariants now specify: "When `phase == NORMAL`: all counters reset to zero. This invariant is enforced on every transition to NORMAL." Interrupted recovery (RECOVERY→DEEP) also specified with `consecutive_recovery` reset.
- [x] CHK007 - Does the CrisisState entity include a field for the profit rate time series (or rolling window) needed by the multi-period detector? **RESOLVED**: FR-020 amended to require only current profit rate (`float | None`) and current CrisisState. The detector accumulates history via consecutive-period counters in CrisisState; no external time series is needed.
- [x] CHK008 - Is the "contributing factors breakdown" attribute of BifurcationRiskMetric defined with specific structure (dict, list, named fields)? **RESOLVED**: Key Entities now states "The three sub-scores (solidarity_density, legitimation, class_burden_ratio) constitute the contributing factors breakdown." No separate field needed.

## Requirement Clarity

- [x] CHK009 - Is the hysteresis recovery period index `k` in FR-009 defined with its starting value? **RESOLVED**: FR-009 now specifies "k starts at 1 for the first recovery period (so the first period recovers to `normal_rate * (1 - h)` = 50% of normal with default h=0.5)."
...
- [x] CHK012 - Is wage compression in FR-016 compounding or linear? **RESOLVED**: FR-016 now specifies compounding: `wage_new = wage_current * (1 - wage_compression_rate)` each deep crisis period. Cumulative compression after k periods: `1 - (1 - wage_compression_rate)^k`, tracked in `CrisisState.cumulative_wage_compression`.
...
- [x] CHK014 - Do the crisis phase names in US2 acceptance scenarios align with the phase definitions in FR-003? **RESOLVED**: US2 AS1 now explicitly references "ONSET or EARLY crisis phase (crisis active for 1-5 periods, i.e. onset at period N through early at N+4)" with specific FR-006 multiplier values (1.5x/2.5x precaritization, 1.2x/1.8x dispossession).
...
- [x] CHK019 - Are the acceptance scenario thresholds in US2 quantified? **RESOLVED**: US2 AS1 now cites exact FR-006 multiplier values: "precaritization rate is amplified per FR-006 (1.5x at onset, 2.5x at early) while dispossession rate amplification is modest (1.2x at onset, 1.8x at early)."
- [x] CHK020 - Is SC-004's solidarity density threshold (60%, 20%) traceable to a requirement? **RESOLVED**: SC-004 now includes traceability note: "verification values derived from the FR-011 formula with default weights (w_s=1.0, w_b=1.0) and neutral legitimation/burden inputs. They validate directional behavior, not configurable parameters."
...
- [x] CHK025 - Is the scenario where crisis deepens while bifurcation risk is near a threshold crossing addressed? **RESOLVED**: FR-022 now defines BIFURCATION_THRESHOLD emission: "Emitted when `|bifurcation_risk.score|` crosses `bifurcation_event_threshold` (default: 0.5, configurable in FR-023)." FR-023 includes `bifurcation_event_threshold` parameter.

## State Machine Completeness

- [x] CHK026 - Are all valid phase transitions enumerated? **RESOLVED**: FR-003 now specifies "Phase progression is strictly linear: NORMAL→ONSET→EARLY→DEEP is the only forward path; there is no early recovery (ONSET→RECOVERY or EARLY→RECOVERY)." data-model.md state machine updated with all transitions and conditions.
- [x] CHK027 - Is the phase transition from ONSET directly to RECOVERY specified? **RESOLVED**: FR-003 explicitly rules this out: "If profit rates recover before reaching DEEP, the county continues through EARLY and DEEP phases anyway." Recovery is only possible from DEEP phase.
- [x] CHK028 - Is the interrupted recovery scenario (edge case #5) specified with enough precision to determine which phase the county returns to? **RESOLVED**: data-model.md CrisisState invariants now specify: "Interrupted recovery (RECOVERY → DEEP): `consecutive_recovery` resets to 0, `crisis_duration` and `cumulative_wage_compression` carry forward from pre-recovery values, `peak_severity` may update if new profit rate is lower." Returns to DEEP, not a duration-based phase.
...
- 28 of 33 items resolved (CHK001, CHK002, CHK003, CHK004, CHK005, CHK006, CHK007, CHK008, CHK009, CHK010, CHK011, CHK012, CHK014, CHK015, CHK016, CHK017, CHK019, CHK020, CHK021, CHK023, CHK025, CHK026, CHK027, CHK028, CHK029, CHK031, CHK032, CHK033). 5 items remain open for implementation-phase resolution (CHK013, CHK018, CHK022, CHK024, CHK030).
