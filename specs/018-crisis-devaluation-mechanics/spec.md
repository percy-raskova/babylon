...
- Q: Which profit rate formula does the crisis detector consume — the flow-based rate `s/(c+v)` from ValueTensor4x3 or the stock-based rate `s/(K+v)` from DerivedRateCalculator? → A: **To be resolved during implementation planning.** The `DerivedRateCalculator` currently has a dimensional inconsistency: `s` and `v` are computed in dollars (via `tau × hours`) while `K` from `CapitalStockCalculator` is in labor-hours. This must be reconciled before the stock-based rate can be used. Additionally, the stock-based rate (where `K ≈ 14.3 × c` at steady state) may produce values an order of magnitude lower than the flow-based rate, requiring `r_threshold` recalibration if the stock-based formula is chosen. See `docs/concepts/piketty-profit-rate.rst` §Dimensional Analysis for full investigation.
