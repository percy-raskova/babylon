...
- Q: Which profit rate formula does the crisis detector consume — the flow-based rate `s/(c+v)` from ValueTensor4x3 or the stock-based rate `s/(K+v)` from DerivedRateCalculator? → A: **Flow-based rate `s/(c+v)` from ValueTensor4x3.** The flow-based rate is dimensionally consistent (all labor-hours), validated at 3-8% against Piketty bounds, and directly compatible with `r_threshold = 0.05`. The stock-based rate has a dimensional inconsistency (dollars + labor-hours in denominator) that is tracked as a Feature 017 bugfix, and even once fixed would produce ~0.5% values requiring a completely different threshold. See `docs/concepts/piketty-profit-rate.rst` §Calibration Epistemology for the full analysis.
...
1. **Given** a county in ONSET or EARLY crisis phase (crisis active for 1-5 periods, i.e. onset at period N through early at N+4) with initial class distribution `{LA: 0.35, Prol: 0.45, Lumpen: 0.15}`, **When** class transitions are computed, **Then** precaritization rate is amplified per FR-006 (1.5x at onset, 2.5x at early) while dispossession rate amplification is modest (1.2x at onset, 1.8x at early), so LA share remains relatively stable compared to deep crisis.
...
As a simulation researcher, I need crisis to follow a lifecycle with distinct phases (normal, onset, early, deep, recovery) rather than being a binary flag, so that the simulation can model the qualitatively different dynamics that characterize each phase of a real economic crisis.
...
- **FR-003**: System MUST classify crisis into phases based on duration: "normal" (no crisis), "onset" (period N), "early" (periods N+1 through N+4, i.e. 4 additional periods matching the typical 1-year lag between crisis onset and deep structural effects at quarterly evaluation cadence), "deep" (period N+5 onward), and "recovery" (profit rate above threshold for M consecutive periods, default M=2). Recovery phase duration is proportional to crisis duration: `recovery_duration = min(crisis_duration, R_cap)` where R_cap is configurable (default: 8 periods). After recovery_duration periods, phase transitions to "normal". Phase progression is strictly linear: NORMAL→ONSET→EARLY→DEEP is the only forward path; there is no early recovery (ONSET→RECOVERY or EARLY→RECOVERY). If profit rates recover before reaching DEEP, the county continues through EARLY and DEEP phases anyway — the consecutive-period counter reflects initial detection, not ongoing below-threshold status. Recovery is only possible from DEEP phase.
...
- **FR-009**: System MUST implement hysteresis in recovery: when transitioning from crisis to recovery phase, upward transition rates (accumulation, stabilization) recover according to a hysteresis coefficient `h` (default: 0.5) such that the effective recovery multiplier at recovery period `k` is `normal_rate * (1 - h^k)`, where `k` starts at 1 for the first recovery period (so the first period recovers to `normal_rate * (1 - h)` = 50% of normal with default h=0.5), approaching but not immediately reaching normal rates.
...
- **FR-016**: System MUST apply wage compression during deep crisis phases, reducing the effective wage rate by a configurable fraction per crisis period (default: 2% per period of deep crisis). Compression is compounding: `wage_new = wage_current * (1 - wage_compression_rate)` each deep crisis period. The cumulative compression after `k` deep periods is `1 - (1 - wage_compression_rate)^k`, tracked in `CrisisState.cumulative_wage_compression`.
...
- **FR-020**: System MUST replace the existing `ThresholdCrisisDetector` with the new `MultiPeriodCrisisDetector`. The new detector removes unemployment rate as a crisis trigger (unemployment is a lagging indicator, a symptom of crisis rather than a cause). Required inputs: current profit rate (`float | None`) and current crisis state (`CrisisState`). The detector accumulates history internally via the CrisisState's consecutive-period counters; no external time series is required. Unemployment-based detection logic is removed, not preserved.
...
- **FR-022**: System MUST emit crisis lifecycle events:
  - **ECONOMIC_CRISIS** (existing): Emitted at crisis onset (NORMAL → ONSET).
  - **CRISIS_PHASE_TRANSITION**: Emitted on every phase change, with payload per FR-004.
  - **DISPOSSESSION_CASCADE**: Emitted when cumulative crisis-driven LA share decline (relative to crisis-start baseline) crosses a milestone threshold. Default milestones: 5, 10, 15 percentage points of LA share decline. Payload: county FIPS, cumulative LA decline, milestone crossed.
  - **BIFURCATION_THRESHOLD**: Emitted when `|bifurcation_risk.score|` crosses `bifurcation_event_threshold` (default: 0.5, configurable in FR-023). Payload: county FIPS, score, direction (revolutionary if negative, fascist if positive).
- **FR-023**: All configurable parameters MUST be defined in `GameDefines` under a new `crisis` category: `crisis_period_ticks` (default: 13), `r_threshold` (default: 0.05), `N` (default: 3), `M` (default: 2), `R_cap` (default: 8), amplification multipliers (per FR-006 table), hysteresis coefficient `h` (default: 0.5), wage compression rate (default: 0.02), wage compression floor ratio (default: 0.8, i.e. 80% of subsistence cost), bifurcation solidarity weight `w_s` (default: 1.0), bifurcation burden weight `w_b` (default: 1.0), class burden epsilon (default: 0.001), bifurcation event threshold (default: 0.5, i.e. |score| >= 0.5 emits BIFURCATION_THRESHOLD), and dispossession cascade milestones (default: [0.05, 0.10, 0.15], i.e. 5/10/15 percentage points of LA share decline).
...
- **BifurcationRiskMetric**: Represents the political trajectory indicator for a county under crisis. Attributes: bifurcation score (-1 to +1), cross-class solidarity density, legitimation index, class burden ratio (LA loss rate / proletariat loss rate). The three sub-scores (solidarity_density, legitimation, class_burden_ratio) constitute the contributing factors breakdown.
- **PhasedAmplificationProfile**: Maps crisis phases to transition rate multipliers. Configurable via GameDefines.
- **CrisisEvent payloads**: Crisis events use the existing `Event` model with crisis-specific payload dicts. Phase transition payloads include: county FIPS, previous phase, new phase, current profit rate, crisis duration. Bifurcation threshold payloads include: county FIPS, score, threshold crossed. Dispossession cascade payloads include: county FIPS, cumulative LA share decline, milestone threshold.
...
- **SC-004**: In a scenario with high cross-class solidarity (>60% edge density), the bifurcation risk metric is below -0.3 (revolutionary trajectory). In a scenario with atomized solidarity (<20% edge density), the metric is above +0.3 (fascist trajectory). *Traceability: The 60%/20% solidarity density thresholds and ±0.3 score expectations are verification values derived from the FR-011 formula with default weights (w_s=1.0, w_b=1.0) and neutral legitimation/burden inputs. They validate directional behavior, not configurable parameters.*
...
- **A-001**: The flow-based profit rate (`r = s / (c + v)`) from ValueTensor4x3 is the primary crisis indicator. This rate is dimensionally consistent (all labor-hours), validated at 3-8% against Piketty empirical bounds, and directly compatible with `r_threshold = 0.05`. The stock-based rate from DerivedRateCalculator is not used for crisis detection (see `docs/concepts/piketty-profit-rate.rst` §Calibration Epistemology).
...
- **Requires Feature 012** (Capital Stock Dynamics): Provides capital stock K for secular TRPF trend analysis (not used for crisis detection, which uses the flow-based rate from ValueTensor4x3).
