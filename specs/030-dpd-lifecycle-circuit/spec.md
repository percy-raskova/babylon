...
1. **Given** a county with pop_D=1000, pop_P=3000, pop_D_prime=500, **When** a simulation tick runs with rate_D_to_P=0.0556 (1/18 years), rate_P_to_D_prime=0.0213 (1/47 years), rate_D_prime_to_death=0.039 (CDC WONDER 2023), **Then** population transitions are applied correctly and dependency_ratio updates to reflect the new distribution.
...
1. **Given** a county with pension_coverage=0.6, ss_replacement=0.4, healthcare_security=0.7, home_ownership=0.65, retirement_confidence=0.5, **When** legitimation index is computed, **Then** the index equals the weighted sum (0.35*0.65 + 0.30*0.7 + 0.20*0.5 + 0.10*0.6 + 0.05*0.4 = 0.6175) and crisis risk is assessed as "STABLE".
...
- **FR-004**: System MUST compute a legitimation index as a weighted composite of pension coverage, social security replacement rate, healthcare security, home ownership rate, and retirement confidence. Default weights: home_ownership=0.35, healthcare_security=0.30, retirement_confidence=0.20, pension_coverage=0.10, ss_replacement=0.05. **Political judgment**: The ranking of component importance (1. home ownership, 2. healthcare security, 3. retirement confidence, 4. pension coverage, 5. social security replacement) reflects an authorial decision about which material conditions most credibly underwrite the D' promise. The specific weight allocations are tunable GameDefines parameters, but the ordinal ranking is a design invariant.
...
- **FR-014**: System MUST derive tunable class mobility parameters from Chetty Opportunity Atlas data (CSV files at /media/user/data/babylon-data/mobility-atlas/). Parameters include: baseline class mobility rate at reference percentiles (P25 anchor, P75 for interpolation), racial mobility gap coefficients (Black-White, Hispanic-White, etc.), employment probability by class origin, and premature mortality rate by class/race. Each derived parameter MUST document its provenance (source table, column, aggregation method) and be exposed as a tunable GameDefines coefficient.
...
- **ClassMobilityParams**: Tunable parameters for intergenerational class mobility, derived from Chetty Opportunity Atlas KFR data. Includes: baseline mobility rate at reference percentiles (P25, P75), racial gap coefficients by demographic group, premature mortality rates by class/race, and D-phase context covariates (Gini, poverty share, employment rate, single-parent fraction, college rate). All parameters include provenance documentation and are exposed via GameDefines for in-game modification.
