...
- **FR-001**: System MUST compute consciousness-weighted solidarity for any SOLIDARITY edge by combining edge resilience with the minimum collective_identity across the connected agents' marginalized community memberships.
...
- **FR-005**: System MUST compute Betti numbers beta_0 (connected components) and beta_1 (independent cycles) on the solidarity subgraph using standard graph algorithms (not requiring external persistent homology libraries).
...
- **FR-008**: System MUST compute a unified bifurcation tendency combining per-axis tendencies, bridge potential, legitimation crisis intensity, and topological resilience into a classification of "revolutionary", "fascist", or "indeterminate".
...
- **FR-012**: System MUST produce a complete BifurcationResult containing: overall tendency, per-axis tendencies, raw edge counts (cross-line, within-line, lateral antagonism, upward antagonism), consciousness-weighted metrics, community bridge metrics, legitimation index, Betti numbers, resilience metrics, equivalence class distribution, critical singletons, and critical cutsets.
- **FR-013**: System MUST treat consciousness weighting as a qualitative filter — assimilated solidarity (low collective_identity) breaks under crisis stress — not as a simple scalar multiplication of edge weight.
