...
| **III.1 No Magic Constants** | ✅ pass — every constant traces: (a) wage ratios → PWT v10.01 extraction (research.md §R1); (b) final demand → BEA Use Table series (research.md §R2); (c) QCEW employment → existing reference SQLite; (d) the 5-year carry-forward look-back is lifted to `LeontiefRentDefines.qcew_carry_forward_max_years` (configurable, defaults documented). |
