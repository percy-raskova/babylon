...
**Periphery-wage source choice (REVISED post-analyze 2026-05-08)**: **Hickel/Sullivan/Zoomkawala (2021) ERDI** time series from `babylon_hickel_final.csv`, ingested into `marxist-data-3NF.sqlite` as a new `fact_hickel_erdi_annual` table at setup. Per-industry wage ratio = `ERDI[year]` uniformly broadcast across BEA Summary industries (v1 simplification). Original PWT plan was rejected because PWT data is not loaded in the reference SQLite and is not in the babylon-data trove (analyze C4 finding); Hickel ERDI is the empirically grounded available substitute that matches the project's documented intent per `notebooklm_to_babylon_mapping.csv` ("ERDI from Hickel; wage data from QCEW"). Constitution III.4 satisfied via R9 (small PATCH-level catalog amendment adding `Hickel_HSZ_Drain` as a `Fixture` data source — version bump `2.6.1` → `2.6.2`). The same CSV provides the SC-004 calibration target (`annual_drain_usd_billions` column), with column-level orthogonality between input and target (see research.md §R9).
...
| **III.1 No Magic Constants** | **Affected** — every numerical constant in the new sources MUST trace to data | ✅ pass: periphery wage ratios trace to Hickel/Sullivan/Zoomkawala (2021) ERDI time series via `fact_hickel_erdi_annual` (post-R1 pivot — see research.md §R1); final demand derives from `fact_bea_national_industry` + `fact_bea_io_coefficient` (no Use Table ingestion; per R2 + R8 schema check); QCEW employment shares trace to QCEW (constitutionally-listed); the 5-year carry-forward look-back is the only tunable, lifted to `LeontiefRentDefines.qcew_carry_forward_max_years` per III.1 (added via Spec 058's `defines/` package shape). |
| **III.2 Falsifiability Required** | **CRITICAL** — every formula needs prediction + null + falsifying observable | ✅ pass: SC-004 makes phi_hour falsifiable against `babylon_hickel_final.csv` `annual_drain_usd_billions` time series (year-resolved, scale-type-explicit per research.md §R8.4) within order-of-magnitude — this IS the falsifying observation. Null hypothesis: phi_hour uniformly zero (the current stub state). Distinguishing observable: per-county `phi_hour` distribution variance is non-trivial AND order-of-magnitude matches the independent Hickel time series. **Caveat**: post-R1 pivot, the input ERDI and the target `annual_drain_usd_billions` come from the same Hickel CSV (different columns, conceptually distinct quantities — see research.md §R9). The orthogonality is at the column level, not the dataset level. |
| **III.4 Data Catalog (Fixture vs Runtime)** | **CRITICAL** — periphery-wage source MUST be in `data-catalog.yaml` per III.4 | ✅ pass — via R9 PATCH-level constitution amendment: new entry `Hickel_HSZ_Drain` added to `data-catalog.yaml` under `International Trade` category, class `Fixture` (the data is a pinned static historical time series, matches the `Chetty_Atlas` precedent). Version bump `2.6.1` → `2.6.2`. Implemented via task T024c (catalog YAML edit) + T024d (SQLite ingestion). |
...
| **III.1 No Magic Constants** | ✅ pass — every constant traces: (a) wage ratios → Hickel ERDI from `fact_hickel_erdi_annual` per R1 (revised 2026-05-08); (b) final demand → derived `y = x − A·x` from `fact_bea_national_industry` + `fact_bea_io_coefficient` per R2 + R8; (c) QCEW employment → existing reference SQLite via `bridge_naics_bea` per R4; (d) the 5-year carry-forward look-back is lifted to `LeontiefRentDefines.qcew_carry_forward_max_years` (configurable, defaults documented). |
| **III.4 Data Catalog** | ✅ pass — PATCH-level amendment per R9: `Hickel_HSZ_Drain` added as `Fixture` data source. Version bump `2.6.1` → `2.6.2`. |
