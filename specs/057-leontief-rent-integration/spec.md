...
- Q: When the periphery-wage source publishes an industry-year ratio < 1.0 (violating FR-002's structural axiom that core wages never fall below periphery wages), how does the source react — reject, clamp, pass through with warning, or two-mode? → A: **Pass through with structured warning** per (industry, year, ratio-value), mirroring FR-008's per-county outlier pattern. The ≥ 1.0 statement in FR-002 becomes an *expected* invariant (calibration signal when violated), not a *hard* invariant (rejection trigger). Rationale: rejecting loses the whole year-vector for one bad row; clamping silently rewrites real data to match an assumed axiom (loses information that the assumption doesn't always hold); pass-through preserves source integrity, surfaces "the structural axiom doesn't hold for this industry-year" as a visible calibration signal, and the downstream rent calculator naturally produces a small negative rent contribution for those industries — still meaningful, if unusual. Aligns with project's "fail loud in logic layer, not infrastructure" principle. FR-002 and US2 Acceptance Scenario 1 updated below.
