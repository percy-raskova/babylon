...
- **SC-004**: For at least one tick year with both complete BEA + QCEW data and a `babylon_hickel_final.csv` row, the computed national-total imperial rent (`phi_hour` summed across counties × employment-hours) is within an order of magnitude of the `annual_drain_usd_billions` value for that year from the Hickel/Sullivan/Zoomkawala 2021 time series (`/media/user/data/babylon-data/babylon_hickel_final.csv`, year-resolved). The implementer documents which `scale_type` (`Intensive` or `Extensive`) is used as the comparison anchor — see research.md §R8.4 for the scale-type discussion. The single-year reference figure is Hickel et al. 2022's drain estimate of approximately $2.8T (Extensive, 2015) or $9.75T (Intensive, 2015); year-resolved checks against the CSV are stricter and operationalized via parameterized tests in `tests/integration/economics/tick/test_imperial_rent_calibration.py`.
