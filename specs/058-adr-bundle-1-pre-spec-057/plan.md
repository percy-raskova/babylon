...
- **R5**: Spec discrepancy reconciliation (spec previously said 25 enums and 7 `create_*_services()`; actuals are 45 enums and 4 `create_*` + 3 `load_*` helpers — the spec has since been corrected per the 2026-05-08 `/speckit.analyze` remediation pass).

  **R5-corrected (2026-05-08 commit-6 finding)**: Implementation of FR-006 surfaced a deeper R5 miss. On inspection, the 4 `create_*_services()` functions are NOT boilerplate — they perform topological dependency resolution (Level 0 → Level 1 → Level 2 → Level 3) that constructs ~16+ classes per call with explicit interdependencies (e.g., `bea_national → melt → throughput`). `SourceRegistry`'s `Callable[[], object]` model only handles parameterless construction; it does not replace dep-graph resolution. The 3 `load_*_series_from_db` helpers are substantive ~80-LOC SQL query builders, not extractable boilerplate. **Net effect**: `SC-004` (factory.py < 150 LOC) is unachievable in Bundle 1 as conceived and is preserved as `xfail` in `tests/unit/economics/test_factory_shims.py::test_factory_loc_under_150` with a not-met-by-design reason. The primary FR-006 deliverable — `SourceRegistry.builtin_economics()` registering the 7 parameterless melt+gamma classes — DOES ship in commit 6, and `create_economics_services()` delegates to `_get_builtin_registry().get(BasketVisibilityCalculator)` for the parameterless subset. A follow-up bundle may revisit the dep-graph refactor when there is design time to think carefully about it.
