...
- Q: Is the `defines.py` category taxonomy a fixed prescription, a starting set, or empirically derived? → A: **Cluster-by-coupling** — the planner runs an import-graph clustering pass over the 42 `*Defines` classes (and over the 45 enum types in `enums.py`) to choose category boundaries. Manual override is allowed only when clustering produces a sub-file that exceeds the 600-LOC cap; the manual split must be documented in `plan.md`. The categories listed in "Key Entities" below are illustrative, not normative.
- Q: Which import-surface accesses must the bundle preserve through the package splits? → A: **Internal-first, full hygiene.** Babylon has no external downstream consumers, so the contract is solely with internal Babylon code (and the spec-057 implementer). The bundle MUST preserve `from babylon.{enums,defines} import X` (the dominant pattern across all 65+90 importers) and `import babylon.{enums,defines} as alias; alias.X` (free via the `__init__.py` re-export idiom). The bundle MUST define `__all__` explicitly in every new `__init__.py` (and in every category sub-module) — this codifies the public surface, lets `mypy --strict` and `ruff` catch accidental leakage, and controls `from X import *` semantics deterministically. The bundle MUST NOT preserve pickle-stable `__qualname__` (Babylon persists Pydantic models via JSON / Postgres rows, not pickle, so qualname change is invisible to running state). Deep imports (`from babylon.config.defines.economy import EconomyDefines`) are *encouraged* for new code (better bytecode-cache locality — a one-line edit to `EconomyDefines` shouldn't invalidate 65 unrelated modules' caches) but the flat re-export remains the canonical public surface; existing call sites are not migrated to deep imports as part of this bundle.
- Q: Does FR-007's "preserving public method signatures" on the new `TickDynamicsSystem` facade cover return types, exception types, and side-effect ordering? → A: **All three preserved; the facade is a behavioral fence.** Return types MUST be the same class (not merely structurally equal — `is` checks, `isinstance` against subclasses, and Pydantic reference-identity in nested model construction all hold). Exception types MUST be in the same class hierarchy (a caller's `except ValueError` block continues to catch what it caught before; sub-component delegation MUST NOT silently swap to `pydantic.ValidationError`). Event-bus emission order MUST be preserved (a sub-component split that re-orders `publish(EventA)` → `publish(EventB)` is a regression). Verified by SC-001 (the existing tick test suite passes, which already exercises return types via `assert isinstance(...)` and event order via `event_bus.get_history()`) plus a new behavioral-snapshot test (`tests/integration/economics/tick/test_facade_behavioral_fence.py`) that runs one full tick from a frozen seed and diffs the returned `WorldState` plus the event-bus history against a frozen baseline.
- Q: How does `CachedSource[T]` treat `NoDataSentinel` results — memoize them as legitimate cache entries, or skip the cache and re-fetch on every miss? → A: **Cache by default, opt-out per-source.** `CachedSource[T]` exposes a class-attribute `cache_negative_results: bool = True`. The default policy memoizes `NoDataSentinel` results (correct for stable-within-session sources: FRED, BEA Use Tables, county FIPS — repeated misses on the same key don't re-hit the upstream). Sources whose `NoDataSentinel` represents *transient* missing data — derivation outputs that a later pass will fill (DPD lifecycle, MELT recomputation, any source whose `_fetch` is itself the *consumer* of another source's still-pending output) — opt out by setting `cache_negative_results = False` on the subclass. New-source authors don't have to reason about the choice for the common case (the safer default applies); the opt-out is a one-line subclass attribute when needed. Test isolation is preserved: `CachedSource` exposes `invalidate(key)` and `clear()` for tests that need to swap mock return values mid-test, and the `SourceRegistry`'s `variant="test"` substitution rebuilds the cache from scratch.
...
- **FR-001**: System MUST split `src/babylon/models/enums.py` into a `src/babylon/models/enums/` package with category sub-files and a re-exporting `__init__.py`. The `__init__.py` MUST declare an explicit `__all__` enumerating every public symbol; each category sub-module MUST also declare `__all__` for its own contents. Every existing `from babylon.models.enums import X` statement across the project continues to resolve unchanged, and `import babylon.models.enums as e; e.X` continues to work.
- **FR-002**: System MUST split `src/babylon/config/defines.py` into a `src/babylon/config/defines/` package with category sub-files (chosen by import-graph clustering per Clarifications 2026-05-08) and a re-exporting `__init__.py`. The `__init__.py` MUST declare an explicit `__all__` enumerating every public symbol; each category sub-module MUST also declare `__all__` for its own contents. Every existing `from babylon.config.defines import X` statement continues to resolve unchanged, `GameDefines()` instantiates with the same shape, and `import babylon.config.defines as d; d.X` continues to work.
...
- **FR-004**: System MUST introduce a new `src/babylon/core/protocol_kit.py` module exposing `DataSource` (Protocol marker), `CachedSource` (LRU + `NoDataSentinel`-aware mixin/ABC with a `cache_negative_results: bool = True` class-attribute per Clarifications 2026-05-08; exposes `invalidate(key)` and `clear()` methods for test mid-test mock swaps), and `SourceRegistry` (type-keyed registry with optional `variant` discrimination, where `variant="test"` substitution rebuilds the cache from scratch).
...
- **FR-007**: System MUST decompose `src/babylon/economics/tick/system.py` (1705 LOC, 33 methods) into a `src/babylon/economics/tick/system/` package containing a facade (≤200 lines) and focused sub-modules (each ≤400 lines), preserving the public class name `TickDynamicsSystem` and its public methods as a *behavioral fence* (per Clarifications 2026-05-08): identical call signatures, identical return-type classes (not merely structurally-equal objects), identical exception class hierarchies, and identical event-bus emission ordering. Verified by SC-001 plus a new behavioral-snapshot test at `tests/integration/economics/tick/test_facade_behavioral_fence.py`.
...
- **FR-011**: System MUST land each of the five user stories as one or more conventional commits, in the order: US5 (OODA dedup, smallest) → US3 (enums + defines splits) → US1 (protocol_kit + economics migration) → US2 (tick/system decomposition) → US4 (BEA mapping typing). This ordering minimizes inter-commit conflict by landing the lowest-blast-radius change first. (See `plan.md` Phase 0 §R1 for the empirical refinement to **7 commits**: US3 splits into US3a `enums` + US3b `defines` because `defines.py` imports from `enums.py`; US1 lands as 3 commits — kit, melt+gamma migration, factory shim; US2 + US4 ship together in commit 7 since both touch `economics/`.)
...
- **`enums/` package (replacing `enums.py`)**: A Python package holding the same 45 enum types as the historical monolith, organised into themed sub-modules whose boundaries are chosen by an import-graph clustering pass (per Clarifications 2026-05-08). Illustrative category names — *topology, social, consciousness, territory, events, legal, community* — are non-normative; the planner's clustering output is authoritative. The `__init__.py` re-exports every symbol so historical import paths continue to resolve.
- **`defines/` package (replacing `defines.py`)**: A Python package holding the same 42 `*Defines` Pydantic models, with `GameDefines` as the assembler facade exposed via `__init__.py` (with composition logic factored into `_assembler.py`) and category-clustered sub-modules whose boundaries are chosen by an import-graph clustering pass (per Clarifications 2026-05-08). Illustrative category names — *crisis, economy, consciousness, struggle, territory, ooda, state_apparatus* — are non-normative; the planner's clustering output is authoritative, with manual override permitted only to keep any sub-file under the 600-LOC cap.
...
- **`SourceRegistry.builtin_economics()` (new method on the registry)**: A single entry-point that registers every existing `Default*` implementation in `economics/`. Replaces the four `create_*_services()` functions in `economics/factory.py` (the file also contains 3 `load_*_series_from_db` helpers; per `plan.md` Phase 0 §R5 those either move to `economics/_db_helpers.py` or inline into the relevant `Default*._fetch` body — planner picks at commit-6 time).
- **`tick/system/` package (replacing `tick/system.py`)**: A package with a `TickDynamicsSystem` facade (≤200 LOC) and focused sub-modules — illustratively `initialization.py`, `national_parameters.py`, `county_distribution.py`, `crisis.py`, `imperial_rent.py` (housing the current stub `_compute_imperial_rent`), `volume_layers.py`, `tensor_helpers.py`, `bifurcation.py`, `transitions.py` — and wired-in references to the existing `tick/smoothing.py` and `tick/derived_rates.py`. (See `plan.md` "Source Code" tree and `research.md` §R4 for the authoritative method-cluster mapping.)
...
- **SC-006**: After the bundle lands, every historical import statement that exercised a symbol from `enums.py` or `defines.py` continues to resolve. Per Clarifications 2026-05-08, this covers (a) `from babylon.{enums,defines} import X`, (b) `import babylon.{enums,defines} as alias; alias.X`, and (c) `from babylon.{enums,defines} import *` controlled by an explicitly-declared `__all__` in each new `__init__.py`. Verified by (i) `pytest --collect-only` collecting the same number of items as before, (ii) `git grep` byte-equality comparisons of the old and new flat-import lines, and (iii) a new dedicated regression test (`tests/unit/test_public_import_surface.py`) that asserts every symbol named in the new `__all__` resolves and that the set is identical to the pre-split symbol set. Pickle-stable `__qualname__` is explicitly **not** required.
