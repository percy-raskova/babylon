...
**Why this priority**: `postgres_runtime.py` (2094 LOC, 53 methods on one class) and `engine/simulation.py` (1048 LOC, 35 methods on one class) are the two largest god-classes left in the codebase. They concentrate critical-path responsibility (persistence + tick orchestration) into single-file edit bottlenecks. The decomposition is the highest-leverage change in Bundle 2 because it both unblocks parallel work and makes targeted unit testing tractable (stubbing one IO sub-component instead of instantiating the whole runtime).
...
When a contributor adds a new `TickEvent` variant or removes an existing one, every observer that consumes events variant-specifically is forced (by `assert_never` exhaustiveness) to acknowledge the change at typecheck time, not at runtime. The hand-rolled `deserialize_event` switch — which currently silently drops unknown variants and which CLAUDE.md flags as a gotcha — is replaced by Pydantic's built-in discriminated-union dispatch. (Note: a pre-flight grep of `engine/observers/` shows that no observer currently uses `match event:` or `isinstance(...)` chains over the Event hierarchy; ADR-004's observer migration therefore *introduces* exhaustive dispatch where it doesn't yet exist, rather than only patching existing match statements. See research.md D7.)
...
**Independent Test**: All 19 leaf Event variants carry a unique `kind: Literal["..."]` field (5 intermediate base classes — `EconomicEvent`, `ConsciousnessEvent`, `StruggleEvent`, `ContradictionEvent`, `TopologyEvent` — and the `SimulationEvent` root do not, as Pydantic discriminated unions dispatch only on leaves). `TickEvent = Annotated[Union[...], Field(discriminator="kind")]` is defined. `WorldState.events: list[TickEvent]` validates correctly: passing a mismatched dict raises `ValidationError`. `git grep deserialize_event` returns zero results after the final commit. Every observer's `match event:` statement type-checks with exhaustiveness verified.

**Acceptance Scenarios**:

1. **Given** all 19 leaf Event variants, **When** they are inspected, **Then** each carries a unique `kind: Literal["..."]` field and shares a common frozen `_EventBase`.
...
3. **Given** an observer that consumes events variant-specifically (post-ADR-004, this dispatch is *introduced* by the migration; per research.md D7, no observer currently uses `match event:` or `isinstance` chains over the Event hierarchy), **When** a hypothetical new variant is added without updating the observer, **Then** mypy flags the missing `case` (via the `assert_never` exhaustiveness pattern) at typecheck time, not at runtime.
...
### User Story 3 - All 22 System implementations share scaffolding and the schema-bug-masking pattern is gone (Priority: P2)
...
3. **Given** all 22 migrated Systems, **When** the existing System unit tests run, **Then** they pass unchanged, except for any test that was implicitly relying on silent default coercion — those tests are updated to provide the required attribute, and the change is documented in the migrating commit message.
...
**Why this priority**: P2. The existing 6 free-function scenario builders (5 in `engine/scenarios.py` plus `create_wayne_county_scenario` in `engine/scenarios_wayne_county.py`) work; this is a quality-of-life improvement for future scenario authoring. Becomes more valuable once spec 057 lands and stakeholders start authoring more scenarios to explore Leontief calibration.

**Independent Test**: All 6 historical scenario builders (`create_two_node_scenario`, `create_high_tension_scenario`, `create_labor_aristocracy_scenario`, `create_imperial_circuit_scenario`, `create_us_scenario`, `create_wayne_county_scenario`) are ported as `Scenario` subclasses with their old free-function names retained as thin shims. The 2 utilities `get_multiverse_scenarios` and `apply_scenario` and the 5 private helpers (`_classify_hex`, `_compute_metro_influence`, `_get_region_name`, `_create_us_territories`, `_assign_tenancy_edges`) are NOT migrated — they remain on the registry or as module-private functions. `mise run sim:run --scenario <existing_scenario_name>` produces byte-identical baseline output to a pre-refactor run for at least one named scenario.

**Acceptance Scenarios**:

1. **Given** the existing 6 free-function scenario builders, **When** they are converted to `Scenario` subclasses with `build_territories`, `build_classes`, `build_relationships` abstract methods implemented, **Then** the existing free-function names continue to work (kept as shims that wrap the subclass `.build()` call).
...
2. **Given** `engine/systems/edge_transition.py` (856 LOC), **When** it is replaced by `engine/systems/edge_transition/` package with `predicates.py` (Pydantic models) and `system.py` (the System class), **Then** the System class continues to be importable from `engine/systems/edge_transition` and the predicate models are accessible via `engine/systems/edge_transition.predicates`.

---

### User Story 6 - Orphan schemas earn their place or stop adding noise (Priority: P3)

The JSON schemas in `src/babylon/schemas/` flagged as graph-orphans (no incoming `$ref` cross-references) by the latest knowledge-graph rebuild are audited. The seven currently named in ADR-006.6 are `culture`, `ideology`, `institution`, `persona`, `sentiment`, `slice-spec`, `narrative_frame`; an eighth was reported in earlier validation warnings but its identity is confirmed by the pre-flight rebuild (tasks.md T004–T005). The audit distinguishes **graph-orphan** (no incoming `$ref` in the schema graph) from **runtime-unused** (no Python loader references the file) — these are not the same: `narrative_frame.schema.json`, for example, is graph-orphan but runtime-loaded via `engine/observers/schema_validator.py`. Disposition per schema: (a) documented standalone-by-design with a clarifying comment in the corresponding Pydantic model, (b) confirmed unused at runtime AND graph-orphan, deleted with a rationale entry in `ai-docs/decisions.yaml`, or (c) updated with a `description` field clarifying its role. A graph-orphan that is loaded at runtime is annotated (option a or c), never deleted.
...
1. **Given** the orphan schemas surfaced by the pre-flight knowledge-graph rebuild (expected 8; actual count confirmed at task time), **When** each is audited along the graph-orphan vs runtime-unused axes, **Then** for each schema there exists exactly one of: (a) a `# standalone schema, no $ref` comment in the Pydantic counterpart at `models/entities/<name>.py`, (b) a deletion commit (only for schemas confirmed as both graph-orphan AND runtime-unused) with rationale in `ai-docs/decisions.yaml`, or (c) a `description` field added to the schema and the corresponding Pydantic model.
...
- ADR-004 step 3 (observer migration) *introduces* `match event:` or `isinstance(...)` dispatch where applicable; no observer currently uses either pattern (research.md D7). The migration audits each of the 7 observers (`causal.py`, `economic.py`, `endgame_detector.py`, `metrics.py`, `persistence_observer.py`, `schema_validator.py`, `session_recorder.py`) and decides per-observer whether to introduce variant-specific dispatch (with `case _: assert_never(event)`) or to consume events uniformly via narrowed input type `list[TickEvent]`. The decision per observer ships in the rollout commit message.
...
- Spec 057 is merged on this branch (commit `2806cdec`); per Spec 057's US5/FR-009, the 18 spec-057 quarantine markers were lifted and the 8 orphan imperial-rent test files were deleted at merge time. New Leontief-pipeline tests (e.g., `tests/unit/economics/tick/test_phi_hour_field_shape.py`) replaced them. ADR-005's `Simulation` decomposition therefore inherits a clean test surface: the snapshot bridge in `simulation.py` reads `phi_hour` from the live Leontief pipeline (no `0.0` stub remains).

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: System MUST replace `src/babylon/persistence/postgres_runtime.py` (2094 LOC, 53 methods) with a `src/babylon/persistence/postgres_runtime/` package containing a `PostgresRuntime` facade (≤200 LOC) and focused IO sub-modules — at minimum `_pool.py`, `tick_io.py`, `archival_io.py`, `spatial_io.py`, `community_io.py`, `trace_io.py` — each ≤400 LOC.
...
- **FR-007**: System MUST split `src/babylon/models/events.py` (1119 LOC) into a `src/babylon/models/events/` package mirroring the package shape established by Bundle 1, with no file >300 LOC and a re-exporting `__init__.py`.
- **FR-008**: System MUST audit each of the 7 observers in `engine/observers/` and, for those that should consume events variant-specifically, introduce `match event:` (with a `case _: assert_never(event)` exhaustiveness clause) or `if isinstance(...)` chains covering every variant explicitly. Observers that consume events uniformly (e.g., `metrics.py` counters) MUST narrow their input type to `list[TickEvent]` so Pydantic's discriminator validation gates assignments. Per research.md D7, no observer currently uses either dispatch pattern, so this requirement *introduces* exhaustiveness checks rather than augmenting existing ones.
- **FR-009**: System MUST introduce a `src/babylon/engine/systems/base.py:SystemBase` ABC with abstract `step(graph, services, context)` and shared `_read(graph, node_id, key, *, required=False)` / `_write(graph, node_id, key, value)` / `_publish(services, event)` helpers, and migrate all 22 System implementations (21 in `engine/systems/` plus `TickDynamicsSystem` in `economics/tick/system/`) to inherit from it.
...
- **FR-012**: System MUST introduce a `src/babylon/engine/scenarios/base.py:Scenario` ABC and port the 6 historical scenario builders (currently free functions in `scenarios.py` and `scenarios_wayne_county.py`) as subclasses, retaining the historical free-function names as thin shims. The 2 utilities (`get_multiverse_scenarios`, `apply_scenario`) are not migrated and remain on the registry.
...
- **FR-014**: System MUST replace `src/babylon/engine/systems/edge_transition.py` (856 LOC) with an `engine/systems/edge_transition/` package containing `predicates.py` (Pydantic models) and `system.py` (the System class, inheriting from `SystemBase` per FR-009).
- **FR-015**: System MUST audit each orphan JSON schema flagged by the pre-flight knowledge-graph rebuild (currently expected: `schemas/entities/{culture,ideology,institution,persona,sentiment}.schema.json`, `schemas/slice-spec.schema.json`, `schemas/narrative/narrative_frame.schema.json`, plus any 8th surfaced by the rebuild) and resolve each to one of: documented standalone status, deletion with rationale in `ai-docs/decisions.yaml`, or addition of a `description` field clarifying its role. The audit MUST distinguish graph-orphan (no incoming `$ref`) from runtime-unused (no Python loader references the file) — only schemas confirmed as BOTH may be deleted; graph-orphans loaded at runtime (e.g., `narrative_frame.schema.json` via `engine/observers/schema_validator.py`) MUST be annotated, never deleted. If the rebuild reveals fewer or more orphans than the 8 named here, the audit scope adjusts accordingly.
...
- **`SystemBase` (new ABC)**: An abstract base class for all 22 simulation Systems, providing shared read/write/publish helpers. The companion `runtime_checkable Protocol System` is retained for structural typing in tests and mocks.
...
- **`events/` package (replacing `events.py`)**: A package holding 19 leaf Event variants plus 5 intermediate base classes (`EconomicEvent`, `ConsciousnessEvent`, `StruggleEvent`, `ContradictionEvent`, `TopologyEvent`) and the `SimulationEvent` root, organised into themed sub-modules (`economic.py`, `consciousness.py`, `struggle.py`, `contradiction.py`, `topology.py`, `system.py`) plus a `_base.py` with `_EventBase`, the intermediate bases, and the `TickEvent` union assembly. Only the 19 leaves participate in the discriminator.
...
- **Orphan schema audit ledger**: A new section in `ai-docs/decisions.yaml` (or an entry per schema in the existing decisions ledger) recording, for each orphan schema surfaced by the pre-flight rebuild, the disposition (kept-standalone / deleted / annotated), the orphan classification (graph-orphan only / graph-orphan AND runtime-unused), and the rationale.
...
- **SC-004**: After the bundle lands, every observer that consumes events variant-specifically includes an `assert_never(event)` exhaustiveness check (or equivalent `isinstance` chain covering every variant); observers that consume events uniformly type their input as `list[TickEvent]`. Verified by mypy passing under `--strict` on `src/babylon/engine/observers/`.
- **SC-005**: After the bundle lands, all 22 System classes inherit from `SystemBase`, verified by `python -c "from babylon.engine.systems import …; assert issubclass(…, SystemBase)"` for each.
...
- **SC-007**: After the bundle lands, `mise run sim:run` against any of the 6 ported scenarios produces byte-identical output to a pre-Bundle-2 baseline run with the same seed, for at least one named scenario verified at full length.
...
- The 6 scenario builders currently in `engine/scenarios.py` and `engine/scenarios_wayne_county.py` produce deterministic baselines that can be byte-compared. The workhorse `imperial_circuit` scenario is verified byte-deterministic (research.md D8); the other 5 are pre-flighted before US4 begins (`tasks.md` T010). If any scenario is non-deterministic at the per-byte level, the SC-007 byte-equality check is relaxed to numeric-equality with a documented tolerance for that scenario.
- The 8 orphan schemas listed in this spec align with the 7 named in ADR-006 item 6.6 plus one inferred from earlier validation warnings; the pre-flight knowledge-graph rebuild (tasks.md T004–T005) confirms the actual count and identities. If the rebuild surfaces additional orphans, they join the audit scope; if it surfaces fewer, FR-015's scope contracts accordingly. The 132-byte `.understand-anything/knowledge-graph.json` file on disk is a git-lfs pointer; `git lfs pull` or a fresh `/understand-anything:understand` rebuild is mandatory before US6 begins.
