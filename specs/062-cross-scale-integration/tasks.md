...
- [X] T075 [US6] Extend `src/babylon/economics/boundary_flow_register.py` with hex-pair dimensional fields `source_node_id: str`, `source_kind: NodeKind`, `dest_node_id: str`, `dest_kind: NodeKind` per R2 / `contracts/boundary_register.yaml`; preserve backward-compat by deprecating any aggregate-only `.record_aggregate(...)` method
