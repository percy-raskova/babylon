...
- [X] T032 [P] [US3] Write integration test for Detroit-Windsor routing in `tests/integration/test_detroit_windsor_routing.py` per quickstart §5: inject one synthetic Canadian-coded row into a session's LODES matrix at session-init time; run one tick; assert exactly one COMMUTE_OUT row with `dest_node_id='canada'`. Mark `@pytest.mark.integration`  *(landed at unit level in test_vol2_classifier_routing.py — 3 tests prove classifier reclassifies Canadian-coded blocks at emission time without integration setup)* *(2026-07-08 — the NAMED integration file now exists (supersedes "landed at unit level"): `tests/integration/test_detroit_windsor_routing.py`, 2 tests — synthetic state-99 row injected into the session's persisted OD matrix routes to exactly one canada COMMUTE_OUT through the classifier at emission time, plus the SC-012 negative control on the same session's pre-injection run.)*
- [X] T033 [P] [US3] Write integration test for FR-026 fail-fast invariant in `tests/integration/test_canada_required_invariant.py`: call `initialize_session(..., external_node_overrides=frozenset(["china", "eu"]), synthetic_lodes_canadian_rows=True)`; assert raises `SessionInitializationError` with message matching `r"canada.*not present"`. **Also assert SC-006 timing**: wrap the failing call with `start = time.perf_counter()` / `elapsed = time.perf_counter() - start`; assert `elapsed < 5.0` (SC-006 fail-fast wall-time budget). Mark `@pytest.mark.integration`  *(FR-026 fail-fast invariant already wired into postgres_initialization during T020 / US1; canada-missing scenarios raise InitializationError)* *(2026-07-08 fold-in decision — SUPERSEDED same day by lane-6.2, see below)* *(2026-07-08 lane-6.2 — the earlier fold-in is REVERSED: the `external_node_overrides` / `synthetic_lodes_canadian_rows` seams were built (see T042 addendum), so the NAMED integration file now exists: `tests/integration/test_canada_required_invariant.py`, 3 tests — (1) canada removed from the registry + synthetic canada rows raises `InitializationError` matching `r"canada.*not present"` in <5s (SC-006, the check runs before any SQLite/Postgres I/O), (2) default registry + synthetic rows completes and persists the synthetic canada OD row, (3) an explicit override that includes canada passes and declares exactly the overridden node set. The exception class is `InitializationError` (spec's `SessionInitializationError` never existed). Postgres-only — no LODES/TIGER data required.)*
...
- [X] T040 [P] [US3] Create `src/babylon/economics/border_commute_synthesis.py` defining `BorderCommuteSynthesisLoader` + `BorderCommuteFlow` (frozen Pydantic) per data-model.md §1.5b + contracts/border_commute_synthesis.yaml. Implement the FR-035 synthesis formula `weekly_commuters[week] = monthly_vehicles[month_containing(week)] × border_commute_share / 4.33`, the `synthesize_year(year)` method, and the `merge_into_year_matrix(matrix, year)` method that returns a NEW immutable `LODESYearMatrix` with synthesized rows added. Add Postgres persist/load methods analogous to `LODESCommuteMatrixLoader`  *(landed at src/babylon/economics/border_commute_synthesis.py — BorderCommuteSynthesisLoader with BTS+StatCan parsers, FR-035 formula, ISO 8601 week→month convention, persist_to_postgres)* *(completion 2026-07-08 lane-6.2 — the `merge_into_year_matrix(matrix, year)` clause was NOT implemented at first landing (see the T042 note below); it now exists: returns a NEW frozen `LODESYearMatrix` with the mean-of-52-weeks canada aggregate added as an EXTERNAL column (identity return when disabled/empty). DRY: it reuses a new module-level `build_year_matrix()` extracted from `LODESCommuteMatrixLoader._build_csr_matrix` (byte-identical CSR assembly, III.7). New `default_tri_county_aggregate_hex()` helper (→ `872ab2c58ffffff`). 4 new unit tests in `test_border_commute_synthesis.py`.)*
...
- [X] T042 [US3] Extend `initialize_session()` in `postgres_initialization.py` to instantiate `BorderCommuteSynthesisLoader` if `defines.enable_border_commute_synthesis` is True; for each year, call `synthesizer.synthesize_year(year)` and `synthesizer.persist_to_postgres()`, then `LODESCommuteMatrixLoader` reads back the merged matrix. Add `border_synthesis_row_count: int = 0` field to `InitializationReport`  *(landed 2026-07-08 — gate lives in `postgres_initialization.py` inside the LODES-kwargs block (synthesis without LODES hydration would merge into an OD table the session never reads; flag without LODES kwargs is a no-op). The flag path is `defines.economy.enable_border_commute_synthesis` (field lives on EconomyDefines, not GameDefines top level). `border_synthesis_row_count` field pre-existed. The "reads back the merged matrix" clause had NO mechanism (`merge_into_year_matrix` from data-model §1.5b was never built despite T040 [X]) — new `BorderCommuteSynthesisLoader.merge_into_postgres_lodes` supplies the FR-035 merge: mean-of-52-weeks annualization into `s000_workers`, us_to_canada direction only (canada_to_us has no LODES analog) — both disclosed design decisions. New init kwargs: `border_bts_csv/border_statcan_csv/border_port_codes/border_aggregate_hex` (last one REQUIRED when enabled). KNOWN GAP: `CrossBorderCommuteClassifier.classify("canada")` hits Rule 4 → rest_of_usa, so the merged row only routes to canada on the no-classifier path; classifier literal-external-id handling is follow-up scope. Verified by `tests/integration/test_synthesis_enabled_disabled.py`.)* *(lane-6.2 addendum 2026-07-08 — the FR-026 `external_node_overrides` / `synthetic_lodes_canadian_rows` init seams were added (fail-fast BEFORE any SQLite read, so SC-006 <5s holds regardless of page-cache state) and thread `effective_international` into `_bootstrap_external_nodes` + `report.external_node_ids`, REVIVING the previously-dead FR-026 data-driven guard (it was unreachable while external_node_ids was hardcoded to the full 9-node set). Verified by `tests/integration/test_canada_required_invariant.py` (T033).)*
- [X] T043 [US3] Implement `PairedCrossBorderEmissionEvaluator` for FR-030c in `src/babylon/persistence/conservation_audit.py` (or sibling module if more appropriate per existing convention): scan the per-tick boundary register for `COMMUTE_OUT` rows with `dest_kind='external'`; for each, verify a paired `TRADE_EDGE` with swapped source/dest and equal magnitude exists in the same tick; emit a `severity='alarm'` audit row per missing pair. Register the evaluator with `ConservationAuditor` at session init  *(landed 2026-07-08 — implemented in `conservation_audit.py` (after the phi-week evaluator); registered in `headless_runner/runner.py` immediately after the phi registration; added to `_DEFAULT_INVARIANTS` (21→22, unit pin updated). DISCLOSED deviation from the literal "one alarm row per missing pair": `conservation_audit_log` PK `(session_id, tick, scale, invariant_name)` (migration 0014) + the scale CHECK (`'external:%'` per migration 0031) make per-pair rows physically impossible, so the evaluator emits ONE aggregate result per external dest — `computed_value` = missing-pair count, `expected_value` = 0.0, residual ≥ 1.0 grades ALARM per FR-046. Invariant name `paired_cross_border_emission` satisfies T055's `LIKE '%paired_cross_border%'` query. 8 unit tests in `tests/unit/persistence/test_paired_cross_border_evaluator.py` + live-emission proof in the T035 integration file.)*
...
- [X] T045 [US3] Commit US3 with conventional commit: `feat(spec-063): land cross-border classifier + Option B border commute synthesis (T080 + FR-031..FR-036)`  *(landed 2026-07-08 lane-6.2 — US3 T042/T043 + the T040 merge completion + T032/T033/T035/T036 integration coverage committed as a series of conventional `feat(spec-063)`/`test(spec-063)` commits on branch `feat/spec-063-tail-r2` (the orchestrator performs the dev merge, not this lane).)*
...
- [X] T047 [P] Add per-tick wall-time instrumentation to `Vol2CirculationStep.step()` via `time.perf_counter()`; expose as `CirculationStepResult.wall_time_ms` (extend the model). Used by SC-007 verification in the walkthrough script's `--benchmark` mode  *(record-reconciliation 2026-07-08 — checkbox lag, NOT new work: the instrumentation already exists in `src/babylon/engine/systems/vol2_circulation.py` — `wall_time_ms: float = 0.0` field on `CirculationStepResult` (line 81), `t0 = time.perf_counter()` at step entry (line 155), `wall_time_ms = (time.perf_counter() - t0) * 1000.0` before return (line 286). Verified against code at flip time; nothing re-implemented.)*
...
> *(2026-07-08 staleness note: the count below predates T058 — the file now
> holds 58 tasks, T001–T058. Left otherwise untouched pending the Phase 6.1
> checkbox-truth sweep.)*
