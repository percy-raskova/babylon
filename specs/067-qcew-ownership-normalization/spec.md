...
- **SC-002**: After spec-067 ingestion AND both the `WHERE ownership_id = 1` and `WHERE industry_id = 1` filters are removed from `hex_hydrator.py` and `county_aggregation.py`, the canonical 520-tick Michigan-Canada simulation produces a trace whose per-county `s/v` rate of profit falls within the **Marx-grounded band `[0.05, 2.0]`** for every county-tick row. (Amended 2026-05-17 from the spec-original `[0.05, 0.50]` — the original cap was an engineering-convenience figure not derived from Marx's published numbers. Marx Vol I Ch 9 documents empirical s/v rates of 153.85% with no theoretical upper bound; Vol III Ch 9 illustrative profit-rate spheres run 5%-40% as long-run stock-based averages but per-tick flow-based p' legitimately runs higher. The 2.0 cap exists only to flag clearly anomalous values like 1000% that would indicate a code bug; it is not a Marxian theoretical claim.)
- **SC-003**: The test `test_state_rate_of_profit_in_relaxed_band` (the spec-066-named test, with its band amended to `[0.05, 2.0]` per SC-002) passes against the post-067 baseline. Empirically validated 2026-05-17: detroit-tri-county 5-tick run produces p' ≈ 0.7465, within the Marx-grounded band.
...
**SC-002 / SC-003 resolved 2026-05-17**: The original `[0.05, 0.50]` cap had no Marxian theoretical grounding — it was an engineering-convenience figure carried forward from earlier specs. After researching Marx Vol I Ch 9 + Vol III Ch 9 (per Persephone's direction), SC-002 / SC-003 were amended to use `[0.05, 2.0]`. Marx documents empirical s/v rates of 153.85% (Manchester 1871) with no theoretical upper bound on s/v, and Vol III Ch 9 illustrative profit rates run 5%-40% as stock-based long-run averages — but the simulation computes per-tick flow-based p', which legitimately runs higher under high-s/v advanced-capitalism conditions. The 2.0 cap is a "clearly anomalous" sanity check, not a Marxian claim. Detroit-tri-county 5-tick run at p' = 0.7465 falls inside the amended band. See `specs/070-qcew-suppression-amendment/spec.md` and ADR045 negative consequences for the broader rationale.
