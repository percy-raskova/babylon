...
- **FR-006**: Per-county employment and wage values summed from `fact_qcew_annual` MUST agree with BLS publication for the same `(area_fips, year)` within ±5 % for every county-year in the 2010-2024 range covered by the canonical Michigan-Canada test scope. *(Superseded 2026-07-02 per spec-097 decision: the target transfers to spec-086 FR-002 — post-imputation reconciliation within ±2 %. Raw post-067 leaf sums are lower-bound estimates; see Known limitation below.)*
...
- **SC-001**: After spec-067 ingestion, `SELECT SUM(employment) FROM fact_qcew_annual WHERE area_fips = '26163' AND year = 2010` (Wayne County) — with no ownership filter AND no industry filter applied — returns a value within ±5 % of the BLS publication for Wayne County 2010 (~660 K). *(Superseded 2026-07-02 per spec-097 decision by spec-086 SC-003: within ±2 % post-imputation.)*
- **SC-002**: After spec-067 ingestion AND both the `WHERE ownership_id = 1` and `WHERE industry_id = 1` filters are removed from `hex_hydrator.py` and `county_aggregation.py`, the canonical 520-tick Michigan-Canada simulation produces a trace where, at terminal tick, `total_s > 0` (surplus value is positive — capitalism is net-productive in labor-hour terms) AND `p' = total_s / (total_c + total_v)` is finite. **No upper cap on p' is asserted**, because Marx (Vol I Ch 9 + Vol III Ch 9) places no theoretical ceiling on s/v or s/(c+v); only physical/legal limits on the working day. (Amended 2026-05-17 from the spec-original `[0.05, 0.50]` band — engineering-cap theater, not Marxism. The `s ≤ 0 with ongoing accumulation` case is interesting in its own right — a rentier-economy / financialization signal — and warrants a separate detection rather than a pass/fail rejection here.)
- **SC-003**: The test `test_state_rate_of_profit_in_relaxed_band` passes structural invariants (s > 0, p' finite) against the post-067 baseline. Empirically validated 2026-05-17: detroit-tri-county 5-tick run produces total_s = 2.26 B (positive ✓), p' = 0.7465 (finite ✓). Test name preserved per FR-009.
...
- **SC-006**: The post-067 michigan-e2e baseline regeneration is reproducible — running the canonical Michigan-Canada simulation twice with the same seed produces trace.csv content that agrees within float64 machine precision (max relative error ≤ 10^-12 across all numeric cells). Empirically validated 2026-05-17: two 520-tick regens (45 min wallclock each) produced 43,160 rows in both files with 16,491 cells differing in last-decimal noise only; **max relative error 7.47 × 10^-14**, max absolute error 2.62 × 10^-3 on `energy_stock` values ~10^4. (Amended from the original "byte-identical content" target, which is unattainable without `PYTHONHASHSEED=0` + single-threaded BLAS. Hash-order-dependent floating-point summation order produces noise at the machine-precision floor, not a determinism failure.)
- **SC-007**: Per-county employment totals summed from `fact_qcew_annual` agree with BLS publication within ±5 % for ≥ 95 % of (county × year) pairs in the 2010-2024 Michigan scope. *(Superseded 2026-07-02 per spec-097 decision by spec-086 SC-001/SC-002: ≥ 99 % of all county-years within ±2 % post-imputation.)*
...
Resolution path *(updated 2026-07-02)*: the amendment spec was renumbered spec-070 → **spec-097**, which is now FINAL: strategy 4 (synthetic imputed totals), generalized to full hierarchical cell reconstruction, is adopted and implemented by **spec-086 (QCEW Loader Reimplementation with Synthetic Suppression Imputation)**. Until spec-086 lands, downstream consumers should treat post-067 `v` and `employment_proxy` values as lower-bound estimates of the BLS publication total (10-30 % low).

**SC-002 / SC-003 resolved 2026-05-17 (two-stage refactor)**:

* **Stage 1** — Recognized the original `[0.05, 0.50]` cap had no Marxian theoretical grounding. Amended to `[0.05, 2.0]` based on Marx Vol I Ch 9 (Manchester 1871 s/v = 153.85% empirically observed; no theoretical upper bound on s/v) and Vol III Ch 9 (illustrative stock-based profit rates 5%-40% as long-run averages; flow-based per-tick p' legitimately higher).
* **Stage 2** — Recognized that ANY numeric upper cap is engineering theater, not Marxism. Marx places no ceiling on s/(c+v) — only physical limits on the working day. Refactored to assert STRUCTURAL invariants instead: `total_s > 0` (capitalism net-productive in labor-hour terms) AND `p' finite` (catches NaN / divide-by-zero). The interesting `s ≤ 0 with ongoing accumulation` case is a rentier / financialization signal worth flagging separately, not pass/fail here.

Detroit-tri-county 5-tick run validates 2026-05-17: total_s = 2.26 B > 0 ✓, p' = 0.7465 finite ✓. Per Persephone's review.
