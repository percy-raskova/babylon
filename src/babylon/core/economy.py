...
class Economy:
    ...
    def simulate_market(self) -> None:
        ...
        demand_change = (
            (1 - self.unemployment_rate/100) * 0.1 +  # Employment effect
            (1 - self.inflation_rate/10) * 0.05    # Price stability effect
        )
    ...
    def apply_policy(self, policy: dict[str, Any]) -> None:
        ...
        if 'wage_adjustment' in policy:
            self.wage_share = max(0.0, min(1.0, 
                self.wage_share + policy['wage_adjustment']
            ))
        ...
        if 'resource_efficiency' in policy:
            self.resource_utilization = max(0.0, min(1.0,
                self.resource_utilization + policy['resource_efficiency']
            ))
