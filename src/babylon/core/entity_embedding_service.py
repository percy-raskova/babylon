...
class EntityEmbeddingService:
    ...
    def embed_entities_batch(self, entities: list[Entity]) -> list[Entity]:
        ...
        try:
            ...
            for embedded, original in zip(embedded_results, entities, strict=False):
    ...
    def search_similar_entities(
        self, query_entity: Entity, n_results: int = 5, include_distances: bool = True
    ) -> list[dict[str, Any]]:
        ...
        if query_entity.embedding is None:
            raise ValueError(
                f"Query entity {query_entity.id} must have embedding for similarity search"
            )
    ...
    def search_by_criteria(
        self, criteria: dict[str, Any], n_results: int = 10
    ) -> list[dict[str, Any]]:
        ...
        try:
            results = self.entity_collection.get(
                where=criteria, limit=n_results, include=["documents", "metadatas"]
            )
    ...
    def remove_embeddings(self, entities: list[Entity]) -> list[Entity]:
        ...
        try:
            ...
            embeddables = [EmbeddableEntity.from_entity(entity) for entity in entities]
            _debedded = self.embedding_manager.debed_batch(embeddables)  # noqa: F841
