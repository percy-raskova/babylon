...
class Politics:
    ...
    def _update_stability(self) -> None:
        ...
        # Update stability
        stability_change = (
            legitimacy_factor + democracy_factor + repression_effect + resistance_effect
        )
    ...
    def _update_power_relations(self) -> None:
        ...
        self.power_relations["working_class"] = max(
            0.0, min(1.0, self.power_relations["working_class"] + consciousness_effect)
        )

        # Update ruling class power based on economic control
        self.power_relations["ruling_class"] = max(
            0.0,
            min(1.0, self.power_relations["ruling_class"] - consciousness_effect * 0.5),
        )
    ...
    def _update_class_dynamics(self) -> None:
        ...
        consciousness_growth = self.repression_level * 0.2 + (1 - self.legitimacy) * 0.1
        self.class_consciousness = max(
            0.0, min(1.0, self.class_consciousness + consciousness_growth * 0.1)
        )

        # Resistance potential based on consciousness and repression
        self.resistance_potential = (
            self.class_consciousness * 0.6 + self.repression_level * 0.4
        ) * (1 - self.legitimacy)
