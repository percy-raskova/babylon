...
from ..models.contradiction import Contradiction, Entity, Effect, Attribute
...
def parse_contradictions(xml_file):
    ...
    for contradiction_elem in root.findall('Contradiction'):
        ...
        name = contradiction_elem.find('Name').text if contradiction_elem.find('Name') is not None else None
        ...
        principal_contradiction = contradiction_elem.find('PrincipalContradiction').text.lower() == 'true'
        ...
        potential_for_transformation = contradiction_elem.find('PotentialForTransformation').text.lower() == 'true'
        conditions_for_transformation = [
            cond.text for cond in contradiction_elem.find('ConditionsForTransformation').findall('Condition')
        ]
        ...
        resolution_conditions = [
            cond.text for cond in contradiction_elem.find('ResolutionConditions').findall('Condition')
        ]
        ...
        contradiction = Contradiction(
            id, name, description, entities, universality, particularity,
            principal_contradiction, principal_aspect, secondary_aspect,
            antagonism, intensity, state, potential_for_transformation,
            conditions_for_transformation, resolution_methods,
            resolution_conditions, effects, attributes
        )
