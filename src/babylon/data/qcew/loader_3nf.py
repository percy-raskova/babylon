...
class QcewLoader(ApiLoaderBase):
    ...
    def _process_api_year(
        ...
        Fetches both state-level aggregates (XX000 format) and county-level
        data (5-digit FIPS) since state endpoints only return CSA data.
        """
        state_iter = tqdm(states, desc=f"States {year}", disable=not verbose)

        for state in state_iter:
            # 1. Fetch state-level aggregates (CSA/metro data)
            self._fetch_state_aggregates(
                session, client, year, state, lookups, counts, stats, verbose
            )

            # 2. Fetch county-level data for this state
            self._fetch_county_data(session, client, year, state, lookups, counts, stats, verbose)

    def _fetch_state_aggregates(
        self,
        session: Session,
        client: QcewAPIClient,
        year: int,
        ...
        """Fetch state-level aggregates (CSA/metro data) for a state."""
        checkpoint_key = f"{state.state_fips}_state"
        if self._is_completed(session, "qcew", year, checkpoint_key, "api", "T"):
            if verbose:
                logger.debug(f"Skipping completed state aggregates: {year}/{state.state_fips}")
            return
        ...
        try:
            ...
            self._mark_completed(session, "qcew", year, checkpoint_key, "api", "T", record_count)
            session.flush()

        except QcewAPIError as e:
            self._handle_api_error(e, area_code, year, stats)

    def _fetch_county_data(
        self,
        session: Session,
        client: QcewAPIClient,
        year: int,
        ...
        """Fetch county-level data for all counties in a state."""
        # Get counties for this state
        counties = session.query(DimCounty).filter(DimCounty.state_id == state.state_id).all()

        for county in counties:
            checkpoint_key = county.fips
            if self._is_completed(session, "qcew", year, checkpoint_key, "api", "T"):
                continue
            ...
            try:
                for record in client.get_area_annual_data(year, county.fips):
                    self._process_api_record(session, record, year, state, lookups, counts)
                    record_count += 1
                    self._maybe_flush(session, counts, verbose)

                self._mark_completed(
                    session, "qcew", year, checkpoint_key, "api", "T", record_count
                )
                session.flush()

            except QcewAPIError as e:
                if e.status_code != 404:  # 404 is expected for some small counties
                    self._handle_api_error(e, county.fips, year, stats)
