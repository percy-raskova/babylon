...
_BUILDERS: dict[EventType, EventBuilder] = {
    ...
    # P25 electoral machine (ADR128) — publishers arrive U8-U12
    EventType.ELECTION_HELD: lambda tick, timestamp, payload: ElectionHeldEvent(
        tick=tick,
        timestamp=timestamp,
        sovereign_id=payload.get("sovereign_id", ""),
        jurisdiction_level=payload.get("jurisdiction_level", ""),
    ...
    EventType.GOVERNMENT_FORMED: lambda tick, timestamp, payload: GovernmentFormedEvent(
        ...
        timestamp=timestamp,
        sovereign_id=payload.get("sovereign_id", ""),
        governing_coalition=payload.get("governing_coalition", ""),
        faction_balance_shift=payload.get("faction_balance_shift", 0.0),
    ),
    EventType.POLICY_ENACTED: lambda tick, timestamp, payload: PolicyEnactedEvent(
        tick=tick,
        timestamp=timestamp,
        sovereign_id=payload.get("sovereign_id", ""),
        policy_axis=payload.get("policy_axis", ""),
        magnitude=payload.get("magnitude", 0.0),
        delivery_ratio=payload.get("delivery_ratio", 1.0),
    ),
    EventType.POLICY_STRUCK: lambda tick, timestamp, payload: PolicyStruckEvent(
        tick=tick,
        timestamp=timestamp,
        sovereign_id=payload.get("sovereign_id", ""),
        policy_axis=payload.get("policy_axis", ""),
        striking_institution=payload.get("striking_institution", ""),
    ),
    EventType.POLICY_PREEMPTED: lambda tick, timestamp, payload: PolicyPreemptedEvent(
        tick=tick,
        timestamp=timestamp,
        sovereign_id=payload.get("sovereign_id", ""),
        policy_axis=payload.get("policy_axis", ""),
        preempting_sovereign=payload.get("preempting_sovereign", ""),
    ),
    EventType.CAPITAL_STRIKE: lambda tick, timestamp, payload: CapitalStrikeEvent(
        tick=tick,
        timestamp=timestamp,
        sovereign_id=payload.get("sovereign_id", ""),
        incidence=payload.get("incidence", 0.0),
        tolerance=payload.get("tolerance", 0.0),
        outflow=payload.get("outflow", 0.0),
    ),
    EventType.DELIVERY_GAP_CROSSED: lambda tick, timestamp, payload: DeliveryGapCrossedEvent(
        tick=tick,
        timestamp=timestamp,
        class_id=payload.get("class_id", ""),
        incumbent_id=payload.get("incumbent_id", ""),
        gap=payload.get("gap", 0.0),
        betrayal_integral=payload.get("betrayal_integral", 0.0),
    ),
    EventType.HOPE_SPIKE: lambda tick, timestamp, payload: HopeSpikeEvent(
        tick=tick,
        timestamp=timestamp,
        class_id=payload.get("class_id", ""),
        hope=payload.get("hope", 0.0),
        platform_id=payload.get("platform_id", ""),
    ),
    EventType.DISILLUSION_WINDOW_OPEN: lambda tick, timestamp, payload: DisillusionWindowOpenEvent(
        tick=tick,
        timestamp=timestamp,
        class_id=payload.get("class_id", ""),
        window_ticks=payload.get("window_ticks", 0),
        bridges_present=payload.get("bridges_present", False),
    ),
    EventType.LEGITIMATION_REFRESH: lambda tick, timestamp, payload: LegitimationRefreshEvent(
        tick=tick,
        timestamp=timestamp,
        territory_id=payload.get("territory_id", ""),
        refresh=payload.get("refresh", 0.0),
        legitimation_index=payload.get("legitimation_index", 0.0),
    ),
    EventType.ELECTIONS_SUSPENDED: lambda tick, timestamp, payload: ElectionsSuspendedEvent(
        tick=tick,
        timestamp=timestamp,
        sovereign_id=payload.get("sovereign_id", ""),
        legitimation_index=payload.get("legitimation_index", 0.0),
    ),
    EventType.POPULAR_FRONT_CALLED: lambda tick, timestamp, payload: PopularFrontCalledEvent(
        tick=tick,
        timestamp=timestamp,
    ...
    EventType.LINE_STRUGGLE_SPLIT: lambda tick, timestamp, payload: LineStruggleSplitEvent(
        ...
        timestamp=timestamp,
        org_id=payload.get("org_id", ""),
        old_stance=payload.get("old_stance", ""),
        new_stance=payload.get("new_stance", ""),
        assets_retained=payload.get("assets_retained", 0.0),
    ),
}
