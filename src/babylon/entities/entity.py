...
class Entity:
    ...
    def add_to_chromadb(self, collection: Any) -> None:
        ...
        if self.embedding is None:
            raise EntityValidationError(
                "Embedding must be generated before adding to ChromaDB", "ENTITY_001"
            )
    ...
    def update_in_chromadb(self, collection: Any) -> None:
        ...
        collection.update(
            ids=[self.id], embeddings=[self.embedding], metadatas=[self.get_metadata()]
        )
    ...
    def find_similar_entities(
        self, collection: Any, n_results: int = 5
    ) -> dict[str, Any]:
        ...
        if self.embedding is None:
            raise ValueError(
                "Embedding must be generated before querying similar entities"
            )
