"""Deterministic record/load for the projection view-models.
...
live tick: a dedicated ``tools/record_*_fixtures.py`` harvester is the only
caller that drives the engine to produce the view-model this module then
records (:mod:`tools.record_projection_fixtures` for
:class:`~babylon.projection.view_models.CountyView`,
:mod:`tools.record_organization_fixture` for
:class:`~babylon.projection.view_models.OrganizationView`, Program 24 P2
WO-18).

Determinism contract: every ``record_*_fixture`` writes ``model_dump(mode="json")``
