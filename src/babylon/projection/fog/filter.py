...
:class:`~babylon.projection.fog.ledger.IntelLedger` entry, which itself ages exact ->
approximate -> unknown per :func:`babylon.projection.fog.ledger.read_intel` — this module
...
``staleness_ticks``/``unknown_ticks`` — :func:`~babylon.projection.fog.ledger.read_intel`
...
**Import-boundary note.** Like :mod:`babylon.projection.fog.reach` and
:mod:`babylon.projection.fog.ledger`, this module imports nothing from ``babylon.*`` and
...
def political_field_group(node_type: str) -> str:
    """The :class:`~babylon.projection.fog.ledger.IntelLedger` ``field_group`` key for a
...
def apply_fog(
    ...
    *,
    ...
    2. **Otherwise, read the ledger once** — ``field_group=f"{node_type}:
       ...
       family, mirroring :class:`~babylon.projection.fog.ledger.IntelEntry`'s own
       "several related fields, one clock" design) via
       :func:`~babylon.projection.fog.ledger.read_intel`. Its tier governs each field
       INDIVIDUALLY:

       * ``"exact"`` or ``"approximate"`` AND the field is a key in the
         reading's ``value_snapshot`` -> use that snapshot value verbatim
         (exact) or as :func:`~babylon.projection.fog.ledger.read_intel` already
    ...
    Args:
        ...
        reach: :func:`babylon.projection.fog.reach.organizing_reach`'s result for this
            session.
        ledger: The session's :class:`~babylon.projection.fog.ledger.IntelLedger`.
        tick: The current simulation tick (fed to
            :func:`~babylon.projection.fog.ledger.read_intel`).
