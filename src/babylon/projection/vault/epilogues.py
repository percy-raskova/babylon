...
Ported verbatim from ``web/game/epilogues.py`` (Program 24 P2 WO-34): the
``EPILOGUES`` dict and its ``Epilogue`` shape are engine-agnostic,
string-keyed data with no import-boundary constraint on this side of the
Hoist, so this is a straight data port, not a re-authoring — the headline,
body, and palette text below are byte-identical to their web-module source.
``tests/unit/projection/vault/test_epilogues.py::TestVerbatimPort`` pins
that equality field-by-field so drift in either copy is caught.

Spec-116 FR-116-4.2 (Playability Spine) killed the ``"THE BUNKER FAILS"`` x4
...
conditionals"): the projection layer looks the recognized pattern up at
render time (:func:`~babylon.projection.vault.render_epilogue.render_epilogue`);
the engine never imports this module; the (flag-off) LLM narrator eulogizes
through its own separate, attributed channel (III.6) — the engine
adjudicates, this module frames, the AI narrates.

**Open question flagged for the BD** (``specs/24-archive/work-orders-p2-p4.md``
OPEN QUESTIONS item 1): the charter (``project/programs/24-the-archive.md``)
says "five epilogue pages"; :class:`~babylon.models.enums.events.GameOutcome`
has six non-``IN_PROGRESS`` members including ``UNRESOLVED``. This module
ships all SIX outcome shells per the work order's own instruction ("senior
ships all six shells; owner rules on the sixth's authorship") — whether
``UNRESOLVED`` is a sixth *authored* epilogue or should collapse to a
generic "no recognized pattern" default remains an open owner ruling, not
resolved here. The horizon-check logic that decides *when* a campaign
resolves to ``UNRESOLVED`` lives in Lane E (WO-39), not this content module.

Source material (per the original web-module docstring, preserved here):
the three crafted (structurally unreachable) Wire triptychs in
``web/game/narrator.py`` (``revolutionary_victory`` / ``ecological_collapse``
/ ``fascist_consolidation``) supplied the voice for those outcomes; the
``red_ogv`` / ``fragmented_collapse`` / ``unresolved`` texts are original to
the epilogues module (no prose existed anywhere for them before spec-116).
"""
...
#: strings — the same case ``_outcome_from_endgame_row`` returns and the
#: (legacy) frontend compares. Keys are string literals (not
#: ``GameOutcome.X.value``) so this module mirrors its web source exactly —
#: a verbatim port, not a re-authoring keyed off the enum. Drift against the
#: enum is caught by ``tests/unit/projection/vault/test_epilogues.py::
#: TestEpiloguesCoverage`` (pins these keys to ``{o.value for o in GameOutcome}
#: - {IN_PROGRESS}``), and drift against the web-module source of truth is
#: caught by ``TestVerbatimPort``.
