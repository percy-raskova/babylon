"""Sandboxed deterministic state-page rendering (Constitution III.13).

Mirrors :mod:`babylon.projection.vault.render` exactly, for
:class:`~babylon.projection.view_models.StateView`. A dedicated module
rather than a shared edit to ``render.py`` (Program 24 P2's Lane-P
shared-file discipline names only ``view_models.py``, ``registry.py``,
``tui/directives.py``, and ``tui/app.py`` as append-only zippers touched by
every Lane-P work order; ``render.py`` deliberately isn't one, so each
entity kind ships its own ``render_<kind>`` module and stays collision-free
against the other parallel Lane-P work orders).
...
custom filters, globals, or finalizers, and templates are loaded from the
``babylon.projection.vault`` package's own ``templates/`` directory via a
:class:`~jinja2.PackageLoader` (the same loader ``render.py`` uses — both
county and state templates live in that one directory), which cannot be
pointed outside the package (no filesystem-escape surface).
...
A present-but-``None`` :class:`~babylon.projection.view_models.StateView`
...
from babylon.projection.view_models import ClassComposition, ConsciousnessSimplex, StateView

if TYPE_CHECKING:
    from jinja2.sandbox import ImmutableSandboxedEnvironment

#: StateView fields that are always present (identity/provenance) — every
#: other declared field is walked for statblock/absence resolution.
_IDENTITY_FIELDS: Final[frozenset[str]] = frozenset({"kind", "state_fips", "verified_tick"})

#: Remedy verb for each optional StateView field, in the "Verb(Noun) to
#: <goal>" register the spike established for {absence} blocks. Keyed by the
#: exact StateView field name so a field added without a remedy entry fails
...
_REMEDY_BY_FIELD: Final[dict[str, str]] = {
    ...
    "sovereign_id": "Claim(Sovereignty) to attribute a CLAIMS edge on every territory",
}


def _optional_field_names() -> tuple[str, ...]:
    """Return StateView's optional (non-identity) field names, in declared order.

    :returns: field names in :class:`~babylon.projection.view_models.StateView`
        declaration order, excluding the always-present identity fields.
    """
    return tuple(name for name in StateView.model_fields if name not in _IDENTITY_FIELDS)


def _statblock_rows(view: StateView) -> tuple[tuple[str, str], ...]:
    ...
    :param view: the state projection to walk.
    :returns: ``(label, value)`` pairs in StateView declaration order.
    """
...
def _absent_fields(view: StateView) -> tuple[tuple[str, str], ...]:
    ...
    for field_name in _optional_field_names():
        ...
        try:
            ...
            msg = f"no remedy text registered for absent StateView field {field_name!r}"
...
def _build_environment() -> ImmutableSandboxedEnvironment:
    ...
    module builds the environment — mirrors
    :func:`babylon.projection.vault.render._build_environment` exactly
    (kept as an independent copy rather than an import, matching this
    module's own no-shared-file rationale above).

    :returns: a sandboxed environment with StrictUndefined, autoescape off
        (the output is Markdown, not HTML), and templates resolved from the
        vault's ``templates/`` directory only.
    """
...
def render_state(view: StateView, *, verified_tick: int) -> str:
    """Render a state dossier page from a projection view-model.
    ...
    template = environment.get_template("state.md.j2")
