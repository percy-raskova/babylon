...
class ContextWindowManager:
    ...
    def add_content(
        self, content_id: str, content: Any, token_count: int, importance: float = 0.5
    ) -> bool:
        ...
        would_exceed_threshold = new_total > (
            self.config.max_token_limit * self.config.capacity_threshold
        )
        ...
        try:
            ...
            self.logger.info(
                f"Added content {content_id} with {token_count} tokens. Total: {self._total_tokens}"
            )
            return True
    ...
    def _remove_content(self, content_id: str) -> bool:
        ...
        self._content.pop(content_id)  # Remove content, value unused
        ...
        self.logger.info(
            f"Removed content {content_id} with {token_count} tokens. Total: {self._total_tokens}"
        )
        return True

    def optimize(self, target_tokens: int | None = None) -> bool:
    ...
    def _optimize_context_window(
        ...
        while tokens_freed < needed_reduction and self._priority_queue:
            ...
            if removed:
                ...
                if self.lifecycle_manager and hasattr(
                    self.lifecycle_manager, "update_object_state"
                ):
                    try:
                        self.lifecycle_manager.update_object_state(
                            content_id, "REMOVED_FROM_CONTEXT"
                        )
                    except Exception as e:
                        self.logger.error(
                            f"Failed to update lifecycle state for {content_id}: {str(e)}"
                        )
