...
class LifecycleManager:
    ...
    def deactivate(self, obj: Any) -> None:
        ...
        if (
            obj_id in self._immediate_context
            or obj_id in self._active_cache
            or obj_id in self._background_context
        ):
    ...
    def set_memory_pressure(self, pressure: float) -> None:
        ...
        if pressure >= 0.9:  # Extreme pressure
            ...
            self._immediate_limit = max(
                6, int(self._base_immediate_limit * pressure_factor)
            )
            self._active_limit = max(30, int(self._base_active_limit * pressure_factor))
            self._background_limit = max(
                60, int(self._base_background_limit * pressure_factor)
            )
        elif pressure >= 0.8:  # High pressure
            pressure_factor = max(0.3, 1.0 - pressure)  # Allow down to 30% capacity
            self._immediate_limit = max(
                8, int(self._base_immediate_limit * pressure_factor)
            )
            self._active_limit = max(40, int(self._base_active_limit * pressure_factor))
            self._background_limit = max(
                80, int(self._base_background_limit * pressure_factor)
            )
            ...
            self._immediate_limit = max(
                25, int(self._base_immediate_limit * (pressure_factor + recovery_boost))
            )
            self._active_limit = max(
                100, int(self._base_active_limit * (pressure_factor + recovery_boost))
            )
            self._background_limit = max(
                200,
                int(self._base_background_limit * (pressure_factor + recovery_boost)),
            )
    ...
    def _rebalance_all_tiers(self) -> None:
        ...
        for obj_id in list(self._immediate_context.keys()):
            if (
                self._access_counts.get(obj_id, 0) <= -5
                and self._priorities.get(obj_id, 0) == 0
            ):
                obj = self._immediate_context.pop(obj_id)
                obj.state = ObjectState.ACTIVE
                self._active_cache[obj_id] = obj
        ...
        for obj_id in list(self._active_cache.keys()):
            if (
                self._access_counts.get(obj_id, 0) <= -8
                and self._priorities.get(obj_id, 0) == 0
            ):
                obj = self._active_cache.pop(obj_id)
                obj.state = ObjectState.BACKGROUND
        ...
        for obj_id in list(self._background_context.keys()):
            if (
                self._access_counts.get(obj_id, 0) >= 3
                or self._priorities.get(obj_id, 0) > 0
            ) and len(self._active_cache) < self._active_limit:
                obj = self._background_context.pop(obj_id)
                obj.state = ObjectState.ACTIVE
                self._active_cache[obj_id] = obj
                self._tier_transitions += 1
        ...
        for obj_id in list(self._active_cache.keys()):
            if (
                self._access_counts.get(obj_id, 0) >= 5
                or self._priorities.get(obj_id, 0) > 0
            ) and len(self._immediate_context) < self._immediate_limit:
                obj = self._active_cache.pop(obj_id)
    ...
    def _check_state_consistency(self) -> None:
        ...
        duplicate_objects = (
            duplicates_immediate_active
            | duplicates_immediate_background
            | duplicates_active_background
        )
        ...
        if duplicate_objects:
            ...
            # Log recovery action
            logger.warning(
                f"Recovered from corrupt state for objects: {duplicate_objects}"
            )
    ...
    def get_metrics(self) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            ...
            avg_memory_pressure=(
                mean(self._memory_pressure_history)
                if self._memory_pressure_history
                else 0.0
            ),
            peak_memory_pressure=self._peak_memory_pressure,
            immediate_context_usage=len(self._immediate_context)
            / self._immediate_limit,
            active_cache_usage=len(self._active_cache) / self._active_limit,
            background_context_usage=len(self._background_context)
    ...
    def _promote_objects_after_pressure(self) -> None:
        ...
        while (
            len(self._immediate_context) < self._immediate_limit and self._active_cache
        ):
    ...
    def _find_demotion_candidate(self, context: OrderedDict) -> Optional[str]:
        ...
        candidates = [
            (obj_id, obj)
            for obj_id, obj in context.items()
            if self._priorities.get(obj_id, 0) == 0
            and self._access_counts.get(obj_id, 0) <= 0
        ]

        if candidates:
            # Sort by access count (ascending)
            sorted_candidates = sorted(
                candidates, key=lambda x: self._access_counts.get(x[0], 0)
            )
        ...
        sorted_items = sorted(
            ...
            key=lambda item: (
                self._priorities.get(
                    item[0], 0
                ),  # Higher priority = less likely to be demoted
                self._access_counts.get(
                    item[0], 0
                ),  # Higher access count = less likely to be demoted
                -self._last_accessed.get(
                    item[0], 0
                ),  # Older access = more likely to be demoted
            ),
        )
        return sorted_items[0][0]

    def _find_promotion_candidate(self, context: OrderedDict) -> Optional[str]:
        ...
        return sorted_items[0][
            0
        ]  # Return the obj_id with highest priority and access count
