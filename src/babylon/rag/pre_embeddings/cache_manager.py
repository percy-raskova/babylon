...
class EmbeddingCacheManager:
    ...
    def get_from_cache(self, content_hash: str) -> list[float] | None:
        ...
        if content_hash in self.cache:
            ...
            self.metrics.record_metric(
                name="cache_hit", value=1, context=f"hash={content_hash[:8]}"
            )

            lookup_time = time.time() - start_time
            self.metrics.record_metric(
                name="cache_lookup_time", value=lookup_time, context="result=hit"
            )

            return embedding
        else:
            self.metrics.record_metric(
                name="cache_miss", value=1, context=f"hash={content_hash[:8]}"
            )

            lookup_time = time.time() - start_time
            self.metrics.record_metric(
                name="cache_lookup_time", value=lookup_time, context="result=miss"
            )
    ...
    def add_to_cache(self, content_hash: str, embedding: list[float]) -> None:
        ...
        if len(self.cache) > self.config.max_cache_size:
            ...
            self.metrics.record_metric(
                name="cache_eviction", value=1, context=f"max_size={self.config.max_cache_size}"
            )
        ...
        self.metrics.record_metric(
            name="cache_operation_time", value=operation_time, context=f"operation={operation}"
        )
    ...
    def clear_cache(self) -> None:
        ...
        self.metrics.record_metric(
            name="cache_clear_time", value=clear_time, context=f"items_cleared={cache_size}"
        )
    ...
    def save_cache(self) -> None:
        ...
        try:
            ...
            raise CacheError(error_msg, error_code="RAG_442") from e
    ...
    def load_cache(self) -> None:
        ...
        try:
            try:
                ...
                self.metrics.record_metric(
                    name="cache_load_info", value=1, context="no_cache_file_found"
                )
        except Exception as e:
            ...
            self.cache = OrderedDict()
            raise CacheError(error_msg, error_code="RAG_443") from e
