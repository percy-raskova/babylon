...
class ChunkingStrategy:
    ...
    def chunk(self, content: str) -> list[str]:
        ...
        if not content:
            raise ChunkingError("Cannot chunk empty content", error_code="RAG_421")
        ...
        if self.config.strategy == "fixed":
            ...
            raise ChunkingError(
                f"Unsupported chunking strategy: {self.config.strategy}", error_code="RAG_423"
            )
    ...
    def chunk_batch(self, contents: list[str]) -> list[list[str]]:
        ...
        self.metrics.record_metric(
            name="batch_chunking_time", value=batch_time, context=f"batch_size={len(contents)}"
        )
