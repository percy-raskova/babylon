...
class PreEmbeddingsManager:
    ...
    def __init__(
        self,
        config: PreEmbeddingsConfig | None = None,
        preprocessor: ContentPreprocessor | None = None,
        ...
        self.preprocessor = preprocessor or ContentPreprocessor(self.config.preprocessing_config)
        self.chunker = chunker or ChunkingStrategy(self.config.chunking_config)
        self.cache_manager = cache_manager or EmbeddingCacheManager(self.config.cache_config)
    ...
    def process_content(self, content: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            self.metrics.record_metric(
                name="pre_embeddings_error", value=1, context=f"error={str(e)[:100]}"
            )

            raise PreEmbeddingError(
                f"Pre-embeddings processing failed: {str(e)}", error_code="RAG_450"
            ) from e
    ...
    def process_batch(self, contents: list[str]) -> list[list[dict[str, Any]]]:
        ...
        try:
            ...
            self.metrics.record_metric(
                name="pre_embeddings_batch_error", value=1, context=f"error={str(e)[:100]}"
            )

            raise PreEmbeddingError(
                f"Pre-embeddings batch processing failed: {str(e)}", error_code="RAG_451"
            ) from e
    ...
    def prepare_for_embedding(self, obj: Any) -> dict[str, Any]:
        ...
        if not hasattr(obj, "content") or not obj.content:
            raise PreEmbeddingError(
                "Object must have non-empty content attribute", error_code="RAG_452"
            )
        ...
        if self.lifecycle_manager and hasattr(obj, "id"):
            try:
                ...
                self.metrics.record_metric(
                    name="lifecycle_manager_error", value=1, context=f"error={str(e)[:100]}"
                )
    ...
    def prepare_batch_for_embedding(self, objects: list[Any]) -> list[dict[str, Any]]:
        ...
        for obj in objects:
            if not hasattr(obj, "content") or not obj.content:
                raise PreEmbeddingError(
                    "All objects must have non-empty content attribute", error_code="RAG_452"
                )
