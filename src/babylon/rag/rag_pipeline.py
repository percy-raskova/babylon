...
from babylon.rag.chunker import DocumentProcessor, DocumentChunk
from babylon.rag.embeddings import EmbeddingManager
from babylon.rag.retrieval import VectorStore, Retriever, QueryResponse
...
class RagPipeline:
    ...
    async def aquery(
        ...
        threshold = similarity_threshold if similarity_threshold is not None else self.config.default_similarity_threshold
    ...
    def clear_collection(self) -> None:
        ...
        try:
            ...
            if results and results.get('ids'):
                ...
                logger.info(f"Cleared {len(chunk_ids)} chunks from collection '{self.config.collection_name}'")
            else:
...
async def quick_ingest_text(content: str, source_id: str, collection_name: str = "quick_rag") -> IngestionResult:
...
async def quick_query(query: str, collection_name: str = "quick_rag", top_k: int = 5) -> QueryResponse:
