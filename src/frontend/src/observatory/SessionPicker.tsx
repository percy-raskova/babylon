 ...
 *
 * Ported from `web/frontend/src/observatory/SessionPicker.tsx` (spec-096)
  ...
  if (sessions.length === 0) {
    return (
      <div role="status" className="p-8 text-center text-sm text-ash">
        ...
        <code className="text-fog">mise run sim:probe</code>) to populate the Observatory.
      </div>
    );
  }

  return (
    <ul className="divide-y divide-rebar" data-testid="session-list">
      {sessions.map((session) => (
        <li key={session.session_id}>
          <button
            type="button"
            onClick={() => onSelect(session)}
            className="flex w-full items-center justify-between px-4 py-3 text-left hover:bg-rebar"
          >
            ...
            <span className="flex flex-col items-end text-xs text-fog">
              <span>
