...
export const TAKEOVER_PANEL_KEYS = [
  ...
  "network",
  // doctrineTree (the Doctrine Tree takeover, Epoch 3 Wave 6) — same
  // mounted-only contract. Since the DoctrineSystem landed (ADR073) the
  // payload carries LIVE per-session state (acquired_ids, tags, TL,
  // study_target_id), so the tick refetch is load-bearing: doctrine
  // advances every tick and the canvas must follow.
  "doctrineTree",
] as const;
