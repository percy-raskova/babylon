...
def test_record_object_access(metrics_collector: MetricsCollector) -> None:
    ...
    try:
        ...
        pytest.fail(f"Failed to access metrics data: {e!s}")
    except Exception as e:
        pytest.fail(f"Unexpected error recording object access: {e!s}")
...
def test_record_token_usage(metrics_collector: MetricsCollector) -> None:
    ...
    try:
        ...
        pytest.fail(f"Invalid token value: {e!s}")
    except Exception as e:
        pytest.fail(f"Unexpected error recording token usage: {e!s}")
...
def test_latency_tracking(metrics_collector: MetricsCollector) -> None:
    ...
    assert latency_stats["db_queries"]["avg"] == sum(test_latencies) / len(
        test_latencies
    )
