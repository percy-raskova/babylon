...
_STATES = [("26", "Michigan", "MI"), ("46", "South Dakota", "SD"), ("09", "Connecticut", "CT")]
...
#: (naics_code, level) — one full range-sector chain (31-33 → 336111/336112)
#: plus a second 6-digit under a sibling 5-digit for apportionment tests,
#: and a plain-sector chain (54 → 541511/541512).
_INDUSTRIES = [
    ("10", 0),
    ("31-33", 2),
    ("336", 3),
...
def seed_qcew_dims(session: Session) -> None:
    ...
    state_ids = {fips: i for i, (fips, _, _) in enumerate(_STATES, start=1)}
    for i, (fips, state_fips, name) in enumerate(_COUNTIES, start=1):
        session.execute(
            text(
    ...
    for i, (code, level) in enumerate(_INDUSTRIES, start=1):
        session.execute(
            text(
    ...
    for i, (own, title, gov, priv) in enumerate(_OWNERSHIPS, start=1):
        session.execute(
            text(
    ...
    for i, year in enumerate(_YEARS, start=1):
        session.execute(
...
def qcew_orm_engine() -> Engine:
    ...
    tables = [NormalizedBase.metadata.tables[name] for name in _QCEW_TABLES]
