...
class TestAssertEntry:
    ...
    def test_has_entity_passes_when_exists(self, basic_state: WorldState) -> None:
        ...
        Assert(basic_state).has_entity(PERIPHERY_WORKER_ID)  # Should not raise
    ...
    def test_entity_returns_entity_assert(self, basic_state: WorldState) -> None:
        ...
        result = Assert(basic_state).entity(PERIPHERY_WORKER_ID)
    ...
    def test_relationship_returns_relationship_assert(self, basic_state: WorldState) -> None:
        ...
        result = Assert(basic_state).relationship(PERIPHERY_WORKER_ID, COMPRADOR_ID)
    ...
    def test_relationship_raises_when_missing(self, basic_state: WorldState) -> None:
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(basic_state).relationship(COMPRADOR_ID, PERIPHERY_WORKER_ID)  # Wrong direction
...
class TestEntityWealth:
    ...
    def test_has_wealth_passes_within_tolerance(self, basic_state: WorldState) -> None:
        ...
        Assert(basic_state).entity(PERIPHERY_WORKER_ID).has_wealth(0.5)  # Should not raise
    ...
    def test_has_wealth_passes_within_custom_tolerance(self, basic_state: WorldState) -> None:
        ...
        Assert(basic_state).entity(PERIPHERY_WORKER_ID).has_wealth(0.51, tolerance=0.02)
    ...
    def test_has_wealth_fails_outside_tolerance(self, basic_state: WorldState) -> None:
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(basic_state).entity(PERIPHERY_WORKER_ID).has_wealth(1.0)
        assert "C001" in str(exc_info.value)
    ...
    def test_is_poorer_than_passes_when_wealth_decreased(
        ...
        later_state = _factory.create_world_state(
            tick=1,
            entities={PERIPHERY_WORKER_ID: poorer_worker, COMPRADOR_ID: owner},
            relationships=[exploitation_edge],
        )

        Assert(later_state).entity(PERIPHERY_WORKER_ID).is_poorer_than(initial_state)
    ...
    def test_is_poorer_than_fails_when_wealth_increased(
        ...
        later_state = _factory.create_world_state(
            tick=1,
            entities={PERIPHERY_WORKER_ID: richer_worker, COMPRADOR_ID: owner},
            relationships=[exploitation_edge],
        )
    ...
    def test_is_richer_than_passes_when_wealth_increased(
        ...
        Assert(later_state).entity(PERIPHERY_WORKER_ID).is_richer_than(initial_state)
    ...
    def test_wealth_unchanged_from_passes_when_unchanged(self, basic_state: WorldState) -> None:
        ...
        Assert(basic_state).entity(PERIPHERY_WORKER_ID).wealth_unchanged_from(basic_state)
    ...
    def test_wealth_unchanged_from_fails_when_changed(
        ...
        later_state = _factory.create_world_state(
            tick=1,
            entities={PERIPHERY_WORKER_ID: changed_worker, COMPRADOR_ID: owner},
            relationships=[exploitation_edge],
        )

        with pytest.raises(AssertionFailed) as exc_info:
            Assert(later_state).entity(PERIPHERY_WORKER_ID).wealth_unchanged_from(initial_state)
...
class TestEntityIdeology:
    ...
    def test_has_ideology_converts_ideological_profile(self, basic_state: WorldState) -> None:
        ...
        Assert(basic_state).entity(PERIPHERY_WORKER_ID).has_ideology(0.0)
    ...
    def test_has_ideology_fails_when_different(self, basic_state: WorldState) -> None:
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(basic_state).entity(PERIPHERY_WORKER_ID).has_ideology(-0.5)
        assert "C001" in str(exc_info.value)
    ...
    def test_has_class_consciousness_passes_when_correct(self, owner: SocialClass) -> None:
        ...
        state = _factory.create_world_state(
            entities={COMPRADOR_ID: owner},
            relationships=[],
        )
        Assert(state).entity(COMPRADOR_ID).has_class_consciousness(0.25)
    ...
    def test_has_class_consciousness_fails_when_different(self, basic_state: WorldState) -> None:
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(basic_state).entity(PERIPHERY_WORKER_ID).has_class_consciousness(0.9)
        assert "C001" in str(exc_info.value)
    ...
    def test_consciousness_increased_from_passes(self, owner: SocialClass) -> None:
        ...
        initial_state = _factory.create_world_state(
            entities={PERIPHERY_WORKER_ID: worker_low, COMPRADOR_ID: owner},
        )
        ...
        later_state = _factory.create_world_state(
            tick=1,
            entities={PERIPHERY_WORKER_ID: worker_high, COMPRADOR_ID: owner},
        )

        Assert(later_state).entity(PERIPHERY_WORKER_ID).consciousness_increased_from(initial_state)
    ...
    def test_consciousness_increased_from_fails_when_decreased(self, owner: SocialClass) -> None:
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(later_state).entity(PERIPHERY_WORKER_ID).consciousness_increased_from(
                initial_state
            )
        assert "C001" in str(exc_info.value)
    ...
    def test_consciousness_unchanged_from_passes_when_same(self, basic_state: WorldState) -> None:
        ...
        Assert(basic_state).entity(PERIPHERY_WORKER_ID).consciousness_unchanged_from(basic_state)
    ...
    def test_consciousness_unchanged_from_fails_when_different(self, owner: SocialClass) -> None:
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(later_state).entity(PERIPHERY_WORKER_ID).consciousness_unchanged_from(
                initial_state
            )
        assert "unchanged" in str(exc_info.value)
...
class TestEntitySurvival:
    ...
    def test_has_p_acquiescence_passes_when_above_min(self) -> None:
        ...
        state = _factory.create_world_state(entities={PERIPHERY_WORKER_ID: worker})

        Assert(state).entity(PERIPHERY_WORKER_ID).has_p_acquiescence(0.5)
    ...
    def test_has_p_revolution_passes_when_above_min(self) -> None:
        ...
        Assert(state).entity(PERIPHERY_WORKER_ID).has_p_revolution(0.4)
    ...
    def test_has_p_revolution_fails_when_below_min(self) -> None:
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(state).entity(PERIPHERY_WORKER_ID).has_p_revolution(0.5)
    ...
    def test_p_acquiescence_is_passes_within_tolerance(self) -> None:
        ...
        Assert(state).entity(PERIPHERY_WORKER_ID).p_acquiescence_is(0.5)
    ...
    def test_p_revolution_is_passes_within_tolerance(self) -> None:
        ...
        Assert(state).entity(PERIPHERY_WORKER_ID).p_revolution_is(0.5)
...
class TestRelationship:
    ...
    def test_has_tension_increased_passes_when_increased(
        ...
        Assert(later_state).relationship(PERIPHERY_WORKER_ID, COMPRADOR_ID).has_tension_increased(
            initial_state
        )
    ...
    def test_has_tension_increased_fails_when_decreased(
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(later_state).relationship(
                PERIPHERY_WORKER_ID, COMPRADOR_ID
            ).has_tension_increased(initial_state)
        assert "0.5" in str(exc_info.value)
    ...
    def test_tension_is_passes_within_tolerance(self, basic_state: WorldState) -> None:
        ...
        Assert(basic_state).relationship(PERIPHERY_WORKER_ID, COMPRADOR_ID).tension_is(0.1)

    def test_tension_is_fails_outside_tolerance(self, basic_state: WorldState) -> None:
        """tension_is() raises when outside tolerance."""
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(basic_state).relationship(PERIPHERY_WORKER_ID, COMPRADOR_ID).tension_is(0.5)
    ...
    def test_has_value_flow_passes_when_positive(self, basic_state: WorldState) -> None:
        ...
        Assert(basic_state).relationship(PERIPHERY_WORKER_ID, COMPRADOR_ID).has_value_flow()
    ...
    def test_has_value_flow_fails_when_zero(self, worker: SocialClass, owner: SocialClass) -> None:
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(state).relationship(PERIPHERY_WORKER_ID, COMPRADOR_ID).has_value_flow()
    ...
    def test_value_flow_is_passes_within_tolerance(self, basic_state: WorldState) -> None:
        ...
        Assert(basic_state).relationship(PERIPHERY_WORKER_ID, COMPRADOR_ID).value_flow_is(5.0)
    ...
    def test_value_flow_is_fails_outside_tolerance(self, basic_state: WorldState) -> None:
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(basic_state).relationship(PERIPHERY_WORKER_ID, COMPRADOR_ID).value_flow_is(10.0)
        assert "value flow" in str(exc_info.value).lower()
...
class TestChaining:
    ...
    def test_assert_methods_return_self(self, basic_state: WorldState) -> None:
        ...
        result = Assert(basic_state).tick_is(0).has_entity(PERIPHERY_WORKER_ID)
        assert isinstance(result, Assert)

    def test_entity_methods_return_self(self, basic_state: WorldState) -> None:
        """EntityAssert methods return self for chaining."""
        result = Assert(basic_state).entity(PERIPHERY_WORKER_ID).has_wealth(0.5).has_ideology(0.0)
        assert isinstance(result, EntityAssert)
    ...
    def test_relationship_methods_return_self(self, basic_state: WorldState) -> None:
        ...
        result = (
            Assert(basic_state)
            .relationship(PERIPHERY_WORKER_ID, COMPRADOR_ID)
            .tension_is(0.1)
            .has_value_flow()
        )
        assert isinstance(result, RelationshipAssert)
    ...
    def test_complex_chaining(self, basic_state: WorldState) -> None:
        ...
        Assert(basic_state).tick_is(0).has_entity(PERIPHERY_WORKER_ID).has_entity(COMPRADOR_ID)
        Assert(basic_state).entity(PERIPHERY_WORKER_ID).has_wealth(0.5).has_ideology(0.0)
        Assert(basic_state).relationship(PERIPHERY_WORKER_ID, COMPRADOR_ID).tension_is(
            0.1
        ).has_value_flow()
...
class TestErrorMessages:
    ...
    def test_relationship_not_found_shows_available(self, basic_state: WorldState) -> None:
        ...
        with pytest.raises(AssertionFailed) as exc_info:
            Assert(basic_state).relationship(COMPRADOR_ID, PERIPHERY_WORKER_ID)
