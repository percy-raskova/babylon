...
def _load_concepts(path: Path) -> list[dict[str, object]]:
    return load_concepts(path.read_bytes())


def _verify(contract: dict[str, object], concepts: list[dict[str, object]]) -> list[str]:
    return verify_all(contract, concepts, FIXTURE.read_bytes(), str(FIXTURE))
...
def test_cli_fixture_digest_binds_the_requested_fixture(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """A --fixture copy with drifted bytes must fail the digest check.

    The digest census binds the bytes actually loaded from the requested
    fixture path, not the pinned repository fixture.
    """
    fixture_path = tmp_path / "tampered.jsonl"
    tampered = FIXTURE.read_text(encoding="utf-8").replace("identifier", "identifer", 1)
    assert tampered != FIXTURE.read_text(encoding="utf-8")
    fixture_path.write_text(tampered, encoding="utf-8")
    monkeypatch.setattr(
        "sys.argv",
        [
    ...
    assert main() == 1
    assert str(fixture_path) in capsys.readouterr().out


def test_required_concepts_cover_the_dossier_display_labels() -> None:
    contract = load_contract(SCHEMA)
    concepts = {row["concept_id"]: row for row in _load_concepts(FIXTURE)}
...
def test_fixture_bytes_and_semantic_digest_match_the_contract() -> None:
    contract = load_contract(SCHEMA)
    concepts = _load_concepts(FIXTURE)
...
def test_unexpected_concept_refuses() -> None:
    contract = load_contract(SCHEMA)
    concepts = copy.deepcopy(_load_concepts(FIXTURE))
    concepts.append(
        {
    ...
    errors = _verify(contract, concepts)
...
def test_fixture_digest_drift_refuses() -> None:
    contract = load_contract(SCHEMA)
    # Bytes that parse to the pinned rows but drift from the pinned digest:
    # the SHA-256 census must refuse before any row is trusted, and the
    # refusal names the bytes actually presented.
    tampered = FIXTURE.read_bytes().replace(b"identifier", b"identifer", 1)
    assert tampered != FIXTURE.read_bytes()

    with pytest.raises(GlossaryConceptsRefusal) as exc_info:
        verify_all(contract, _load_concepts(FIXTURE), tampered, "tampered.jsonl")

    assert exc_info.value.code == "fixture_digest"
    assert "tampered.jsonl" in str(exc_info.value)
...
def test_compiled_contract_drift_refuses() -> None:
    ...
    with pytest.raises(GlossaryConceptsRefusal) as exc_info:
        _verify(contract, _load_concepts(FIXTURE))
