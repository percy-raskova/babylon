...
1. ``_bridge_economics_overrides`` now constructs a real
   ``throughput_calculator`` and includes it in the overrides dict — unit
   level only (empty ``fips_codes``, so no reference-DB hydration runs; see
   ``_build_capital_calculator``'s gate) to respect the "tests never touch
   the reference DB" constraint for this lane.
