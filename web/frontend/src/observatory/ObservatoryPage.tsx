 ...
 * deep diagnostic panes, for the selected source (live | archive).
 *
      ...
      <main className="min-h-0 flex-1 overflow-auto">
        ...
        {loadState === "ready" && selected !== null && (
          <div className="flex h-full flex-col">
            <div className="flex shrink-0 items-center gap-2 border-b border-soot px-4 py-2">
              <button
                type="button"
                onClick={() => setSelected(null)}
                className="text-xs text-silver hover:text-gold"
              >
                ← sessions
              </button>
              {(["series", "diagnostics"] as DetailView[]).map((d) => (
                <button
                  ...
                  className={`rounded px-2 py-0.5 text-[11px] uppercase tracking-wider ${
                    detail === d ? "bg-dark-metal text-gold" : "text-ash hover:text-silver"
                  }`}
                >
                  {d}
                </button>
