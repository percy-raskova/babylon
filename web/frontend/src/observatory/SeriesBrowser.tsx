    ...
    <div className="flex h-full flex-col gap-3 p-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select
          ...
          className="rounded border border-soot bg-void px-2 py-1 text-xs text-bone"
        >
          {SCOPES.map((s) => (
        ...
        {scope !== "national" && (
          <input
            ...
            className="w-40 rounded border border-soot bg-void px-2 py-1 text-xs text-bone"
          />
        )}
          ...
          {SERIES_METRICS.map((metric) => (
            <button
              ...
              className={`rounded px-2 py-0.5 text-[10px] uppercase tracking-wider ${
                metrics.includes(metric) ? "bg-dark-metal text-gold" : "text-ash hover:text-silver"
              }`}
            >
        ...
        <button
          ...
          className="ml-auto rounded border border-wet-concrete px-3 py-1 text-xs text-silver hover:border-silver disabled:opacity-40"
        >
          Export CSV
        </button>
      </div>
